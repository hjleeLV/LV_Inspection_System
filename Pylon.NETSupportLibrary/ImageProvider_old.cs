...
namespace PylonC.NETSupportLibrary
{
    ...
    public class ImageProvider
    {
        ...
        public bool m_grabThreadRun = false;            /* Indicates that the grab thread is active.*/
        ...
        public void Stop()
        {
            if (m_open && m_grabThread.IsAlive) /* Only start when open and grabbing. */
            {
                ...
                m_grabThread.Abort(); /* Wait for it to stop. */
                //m_grabThread.Join();
            }
        }
        ...
        protected void Open(PYLON_DEVICE_HANDLE device)
        {
            try
            {
                ...
                if (Pylon.DeviceFeatureIsWritable(m_hDevice, "AutoPacketSize"))
                {
                    /* Disable the chunk mode. */
                    Pylon.DeviceSetBooleanFeature(m_hDevice, "AutoPacketSize", true);
                }
                ...
                if (!m_LineCam_mode) // Area카메라이면
                {
                    if (!m_Trigger_mode)
                    {
                        Pylon.DeviceSetBooleanFeature(m_hDevice, "AcquisitionFrameRateEnable", true);
                        Pylon.DeviceSetFloatFeature(m_hDevice, "AcquisitionFrameRateAbs", 15.0);

                        //Pylon.DeviceFeatureFromString(m_hDevice, "AcquisitionFrameRateEnable", "true");
                        //Pylon.DeviceFeatureFromString(m_hDevice, " AcquisitionFrameRateAbs", "10.0");
                        ...
                        //if (Pylon.DeviceFeatureIsAvailable(m_hDevice, "EnumEntry_TriggerSelector_LineStart"))
                        //{
                        //    Pylon.DeviceFeatureFromString(m_hDevice, "TriggerSelector", "LineStart");
                        //    Pylon.DeviceFeatureFromString(m_hDevice, "TriggerMode", "Off");
                        //}
                    ...
                    {
                        Pylon.DeviceSetBooleanFeature(m_hDevice, "AcquisitionFrameRateEnable", false);
                        ...
                        if (Pylon.DeviceFeatureIsAvailable(m_hDevice, "EnumEntry_TriggerSelector_FrameStart"))
                        {
                            Pylon.DeviceFeatureFromString(m_hDevice, "TriggerSelector", "FrameStart");
                            Pylon.DeviceFeatureFromString(m_hDevice, "TriggerMode", "On");
                            //Pylon.DeviceFeatureFromString(m_hDevice, "TriggerDelayAbs", m_Trigger_Delay.ToString());
                        }

                        if (Pylon.DeviceFeatureIsAvailable(m_hDevice, "LineDebouncerTimeAbs"))
                        {
                            Pylon.DeviceSetFloatFeature(m_hDevice, "LineDebouncerTimeAbs", 100.0);
                        }
                    }
                } // 라인카메라이면
                {
        ...
        protected void Grab()
        {
            ...
            try
            {
                ...
                while (m_grabThreadRun) /* Is set to false when stopping to end the grab thread. */
                {
                    ...
                    //if (!Pylon.WaitObjectWait(m_hWait, 1000 * 60 * 60 * 36))
                        if (!Pylon.WaitObjectWait(m_hWait, 60000))
                        {
                        lock (m_lockObject)
                        {
        ...
        protected void EnqueueTakenImage(PylonGrabResult_t grabResult)
        {
            ...
            else /* Conversion is required. */
            {
                ...
                Pylon.ImageFormatConverterSetOutputPixelFormat(m_hConverter, m_converterOutputFormatIsColor ? EPylonPixelType.PixelType_BGR8packed : EPylonPixelType.PixelType_Mono8);
