...
namespace LV_Inspection_System.GUI.Control
{
    public partial class Ctr_ROI : UserControl
    {
        ...
        private void pictureBox_RImage_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            else
            {
                if (pictureBox_RImage.Image != null)
                {
                    ...
                    if (LVApp.Instance().m_Config.m_SetLanguage == 0)
                    {//한국어
                        cm.MenuItems.Add("ROI 마스크 설정", new EventHandler(Masking_ROI));
                    ...
                    {//영어
                        cm.MenuItems.Add("ROI Masking", new EventHandler(Masking_ROI));
        ...
        private void Masking_ROI(object sender, EventArgs e)
        {
            listBox1_SelectedIndex = listBox1.SelectedIndex;
            if (listBox1_SelectedIndex < 0 || pictureBox_Image.Image == null)
            {
                return;
            }
            Form fc = System.Windows.Forms.Application.OpenForms["Form_Masking"];
            if (fc != null)
            {
                fc.Close();
            }

            Form_Masking t_Form_Masking = new Form_Masking();
            if (pictureBox_Image.Image == null)
            {
                return;
            }
            t_Form_Masking.Cam_num = Cam_Num;
            t_Form_Masking.Roi_Idx = listBox1_SelectedIndex;

            t_Form_Masking.Save_Path = LVApp.Instance().excute_path + "\\Models\\" + LVApp.Instance().m_Config.m_Model_Name + "\\" + "CAM" + Cam_Num.ToString("0") + "_ROI" + listBox1_SelectedIndex.ToString("00") + "_Mask.bmp";
            if (File.Exists(t_Form_Masking.Save_Path))
            {
                using (Mat t_load = Cv2.ImRead(t_Form_Masking.Save_Path, ImreadModes.Unchanged))
                {
                    if (t_load.Width != pictureBox_Image.Image.Width || t_load.Height != pictureBox_Image.Image.Height)
                    {
                        File.Delete(t_Form_Masking.Save_Path);  // 존재하는 파일 삭제
                    }
                }
            }

            System.Drawing.Point locationOnForm = pictureBox_Image.FindForm().PointToClient(pictureBox_Image.Parent.PointToScreen(pictureBox_Image.Location));

            t_Form_Masking.Top = locationOnForm.Y;
            t_Form_Masking.Left = locationOnForm.X;
            t_Form_Masking.Width = pictureBox_Image.Width;

            double Img_W = (double)pictureBox_Image.Image.Width;
            double Img_H = (double)pictureBox_Image.Image.Height;

            double Rw = (double)pictureBox_Image.Width / Img_W;
            double Rh = (double)pictureBox_Image.Height / Img_H;

            Rectangle t_ROI = new Rectangle((int)(Convert.ToDouble(dataGridView1.Rows[1].Cells[1].Value)/Rw), (int)(Convert.ToDouble(dataGridView1.Rows[2].Cells[1].Value) / Rh), (int)(Convert.ToDouble(dataGridView1.Rows[3].Cells[1].Value) / Rw), (int)(Convert.ToDouble(dataGridView1.Rows[4].Cells[1].Value) / Rw));

            if (t_ROI.Width % 4 == 1)
            {
                t_ROI.Width -= 1;
            }
            else if (t_ROI.Width % 4 == 2)
            {
                t_ROI.X += 1;
                t_ROI.Width -= 2;
            }
            else if (t_ROI.Width % 4 == 3)
            {
                t_ROI.X += 1;
                t_ROI.Width -= 3;
            }
            ...
            else if (t_ROI.Height % 4 == 2)
            {
                t_ROI.Y += 1;
                t_ROI.Height -= 2;
            }
            else if (t_ROI.Height % 4 == 3)
            {
                t_ROI.Y += 1;
                t_ROI.Height -= 3;
            }
            ...
            t_Form_Masking.t_ROI = t_ROI;
            t_Form_Masking._CAM_Image = pictureBox_Image.Image.Clone() as Bitmap;
            t_Form_Masking.Show();
        }
        ...
        private void pictureBox_Image_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (pictureBox_Image.Image != null)
            {
                ...
                if (tt_x >= 0 && tt_x < pictureBox_Image.Image.Width && tt_y >= 0 && tt_y < pictureBox_Image.Image.Height)
                {
                    ...
                    {
                        t_xyg = "[G:" + grayScale.ToString("000") + "(X" + tt_x.ToString() + ",Y" + tt_y.ToString() + ")]";
                    }
                    else
                    {
                        t_xyg = "[RGB:" + pixelcolor.R.ToString() + "," + pixelcolor.G.ToString() + "," + pixelcolor.B.ToString() + "(X" + tt_x.ToString() + ",Y" + tt_y.ToString() + ")]";
                    }
                    pictureBox_Image.Refresh();
                }
            }
        ...
        public void Change_Initial_Parameter(object sender, EventArgs e)
        {
            ...
            if (tstr == "좌측 끝 기준" || tstr == "우측 끝 기준" || tstr == "좌상 기준" || tstr == "좌하 기준" || tstr == "우상 기준" || tstr == "우하 기준" || tstr == "중심 기준" || tstr == "상부 중심 기준" || tstr == "상부 꼭지점 기준"
                || tstr == "Left end" || tstr == "Right end" || tstr == "Left top" || tstr == "Left bottom" || tstr == "Right top" || tstr == "Right bottom" || tstr == "Center" || tstr == "Top Center"
                )
            {
        ...
        public void Referesh_Select_Menu(bool text_maintain)
        {
            try
            {
                ...
                {//한국어
                    ...
                    dgvCmbCell5.Items.Add("Color CH1 v1~v2 사이");
                    dgvCmbCell5.Items.Add("Color CH2 v1~v2 사이");
                    ...
                    else if (tstr == "Color CH1 v1~v2 사이" || tstr == "Color CH1 v1~v2")
                    {
                        dataGridView1.Rows[6].Visible = true;
                        dataGridView1.Rows[7].Visible = true;
                    }
                    else if (tstr == "Color CH2 v1~v2 사이" || tstr == "Color CH2 v1~v2")
                    {
                        dataGridView1.Rows[6].Visible = true;
                        dataGridView1.Rows[7].Visible = true;
                    }
                ...
                {//영어, 중국어
                    ...
                    dgvCmbCell5.Items.Add("Color CH1 v1~v2");
                    dgvCmbCell5.Items.Add("Color CH2 v1~v2");
        ...
        private void pictureBox_RImage_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (t_AI_Result.Length > 0)
            {
                ...
                {
                    e.Graphics.DrawString(t_AI_Result, myFont, Brushes.Orange, new System.Drawing.Point(15, pictureBox_RImage.Height - 30));
                }
            }
        }
