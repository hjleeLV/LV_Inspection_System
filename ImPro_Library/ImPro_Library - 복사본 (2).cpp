...
void CImPro_Library::Set_Image(unsigned char* src_u8, long size_x, long size_y, long channel, int Cam_num)
{
	...
	if (channel==3)
	{
		...
		Src_Img[Cam_num] = src.clone();//Mat(size_y, size_x, CV_8UC3, src_u8);
		Gray_Img[Cam_num] = Mat(src.size(), CV_8UC1);
		cvtColor(src,Gray_Img[Cam_num],CV_BGR2GRAY);
		Rotation_Calibration(Cam_num);
		Dst_Img[Cam_num] = src.clone();//Mat(size_y, size_x, CV_8UC3, src_u8);
	} else if (channel == 1)
	{
		Mat src = Mat(size_y, size_x, CV_8UC1, src_u8);
		Gray_Img[Cam_num] = src.clone();//Mat(size_y, size_x, CV_8UC1, src_u8);
		Rotation_Calibration(Cam_num);
		Dst_Img[Cam_num] = Mat(src.size(), CV_8UC3);
...
void CImPro_Library::Set_Image_0(unsigned char* src_u8, long size_x, long size_y, long channel, int Cam_num)
{
	...
	if (channel==3)
	{
		...
		Gray_Img[Cam_num] = Mat(Src_Img[Cam_num].size(), CV_8UC1);
	...
	} else if (channel == 1)
	{
		...
		Rotation_Calibration(Cam_num);
		Dst_Img[Cam_num] = Mat(Src_Img[Cam_num].size(), CV_8UC3);
	...
	{// 유레시스 있으면
		...
		{	
			...
			if (m_Cam_FoundPattern[Cam_num].size() > 0)
			{
				...
				if (BOLT_Param[Cam_num].nFindPostProcessing[0] == 0)
				{
				...
				else
				{
					...
					Mat warp_img;
					warpAffine( temp_img, warp_img, warp_mat, Gray_Img[Cam_num].size(),INTER_CUBIC, BORDER_WRAP,CV_RGB(255,255,255));
					BOLT_Param[Cam_num].Gray_Obj_Img = Mat::zeros(temp_img.size(), CV_8UC1);

					int offsetx =  BOLT_Param[Cam_num].Object_Postion.x - c_x;
					int offsety = BOLT_Param[Cam_num].Object_Postion.y - c_y;

					if (max(abs(offsetx), abs(offsety)) > 1)
					{
						Mat trans_mat = (Mat_<double>(2,3) << 1, 0, offsetx, 0, 1, offsety);
						warpAffine(warp_img,BOLT_Param[Cam_num].Gray_Obj_Img,trans_mat,BOLT_Param[Cam_num].Gray_Obj_Img.size(),INTER_CUBIC, BORDER_WRAP,CV_RGB(255,255,255));
						c_x += offsetx;
						c_y += offsety;
					}
					else
					...
					{//후처리 흰색 대상물
						Mat Binary_img;
						threshold(warp_img(BOLT_Param[Cam_num].nRect[0]),Binary_img,127,255,CV_THRESH_BINARY | CV_THRESH_OTSU);
						J_Delete_Boundary(Binary_img,5);
						J_Fill_Hole(Binary_img);
						dilate(Binary_img,Binary_img,element,Point(-1,-1), 3);
						erode(Binary_img,Binary_img,element,Point(-1,-1), 3);

						imwrite("00.bmp",Binary_img);
						vector<vector<Point> > contours;
						vector<Vec4i> hierarchy;
						findContours( Binary_img.clone(), contours, hierarchy, CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE );
						int m_max_object_num = -1;int m_max_object_value = 0;
						RotatedRect minRect;
						Point2f rect_points[4];
						...
						{  
							//if (hierarchy[k][3] == -1)
							{
								...
								tt_rect = minRect.boundingRect();
								if (m_max_object_value <= tt_rect.width*tt_rect.height)
									//&& tt_rect.x > 10 && tt_rect.x+tt_rect.width < Binary_img.cols-10
									//&& tt_rect.y > 10 && tt_rect.y+tt_rect.height < Binary_img.rows-10)
								{
						...
						if (m_max_object_num >= 0)
						{
							...
							{
								drawContours(Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[0]), contours, m_max_object_num, CV_RGB(0,255,0), 1, 1, hierarchy);
								for( int j = 0; j < 4; j++ )
								{
									line( Dst_Img[Cam_num], Point2f(rect_points[j].x + BOLT_Param[Cam_num].nRect[0].x,rect_points[j].y + BOLT_Param[Cam_num].nRect[0].y) , 
										Point2f(rect_points[(j+1)%4].x + BOLT_Param[Cam_num].nRect[0].x,rect_points[(j+1)%4].y + BOLT_Param[Cam_num].nRect[0].y),CV_RGB(255,255,0), 1, 8 );
								}							
								CString msg;
								msg.Format("%1.3f",minRect.angle);
								putText(Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[0]), msg.GetBuffer(), Point(minRect.center.x,minRect.center.y), fontFace, 0.7, CV_RGB(255,100,0), 1, 8);
								imwrite("01.bmp",Dst_Img[Cam_num]);
							}

							//minRect = minAreaRect(contours[m_max_object_num]);
							//Mat warp_mat2 = cv::getRotationMatrix2D(Point2f(minRect.center.x,minRect.center.y), 180.0f*minRect.angle/CV_PI, 1.0f);
							//Mat warp_img2;
							//imwrite("01.bmp",warp_img);
							//warpAffine( warp_img, warp_img2, warp_mat2, Gray_Img[Cam_num].size(),INTER_CUBIC, BORDER_WRAP,CV_RGB(255,255,255));
							//imwrite("02.bmp",warp_img2);
							//BOLT_Param[Cam_num].Gray_Obj_Img = Mat::zeros(temp_img.size(), CV_8UC1);
							//BOLT_Param[Cam_num].Gray_Obj_Img = warp_img.clone();
						}
					}
		      ...
		      Point2d & pt2, float & theta )
{
  ...
  CvPoint maxloc;maxloc.x = maxloc.y = 0;
  CvPoint minloc;minloc.x = minloc.y = 0;
...
bool CImPro_Library::RUN_Algorithm_CAM(int Cam_num)
{
	...
	Alg_Run_Check[Cam_num] = true;BOLT_Param[Cam_num].nUse[0] = 1;
	if (!Gray_Img[Cam_num].empty())
	{
		Result_Info[Cam_num] = "";
		CString msg;
		...
		for (int s=0;s<41;s++)
		{
			...
			if (m_Text_View[Cam_num] && !ROI_Mode)
			{
				if (s > 0)
				{
					rectangle(Dst_Img[Cam_num],BOLT_Param[Cam_num].nRect[s],CV_RGB(0,255,30),2);
				}
				else
				{
			...
			if (BOLT_Param[Cam_num].nBlurFilterCNT[s] > 0)
			{
				...
				blur(CP_Gray_Img_tmp, CP_Gray_Img, Size(BOLT_Param[Cam_num].nBlurFilterCNT[s]*2+1,BOLT_Param[Cam_num].nBlurFilterCNT[s]*2+1));
			}
			if (BOLT_Param[Cam_num].nMethod_Direc[s] == 0 || BOLT_Param[Cam_num].nMethod_Direc[s] == 1)
			{
				Mat CP_Gray_Img_tmp = CP_Gray_Img.clone();
				medianBlur(CP_Gray_Img_tmp, CP_Gray_Img,3);
			}
			...
			{
				Mat CP_Gray_Img_tmp = CP_Gray_Img.clone();
				blur(CP_Gray_Img, CP_Gray_Img_tmp, Size(3,3));
				// Run the edge detector on grayscale
				Canny(CP_Gray_Img_tmp, Out_binary, BOLT_Param[Cam_num].nThres_V1[s], BOLT_Param[Cam_num].nThres_V2[s], 3);
			}
					...
					else
					{
						...
						{// TOP, BOTTOM
							...
							if (BOLT_Param[Cam_num].nROI0_FilterSize[0] > 0)
							{
								...
								{
									Mat Morph_Out_binary = Out_binary.clone();
									erode(Out_binary,Morph_Out_binary,element,Point(-1,-1), BOLT_Param[Cam_num].nROI0_FilterSize[0]);
									dilate(Morph_Out_binary,Out_binary,element,Point(-1,-1), BOLT_Param[Cam_num].nROI0_FilterSize[0]);
								}
								else if (BOLT_Param[Cam_num].nROI0_FilterDirection[0] == FILTER_DIRECTION::VERTICAL)
								{
									Mat Morph_Out_binary = Out_binary.clone();
									erode(Out_binary,Morph_Out_binary,element_v,Point(-1,-1), BOLT_Param[Cam_num].nROI0_FilterSize[0]);
									dilate(Morph_Out_binary,Out_binary,element_v,Point(-1,-1), BOLT_Param[Cam_num].nROI0_FilterSize[0]);
								}
								else if (BOLT_Param[Cam_num].nROI0_FilterDirection[0] == FILTER_DIRECTION::HORIZONTAL)
								{
									Mat Morph_Out_binary = Out_binary.clone();
									erode(Out_binary,Morph_Out_binary,element_h,Point(-1,-1), BOLT_Param[Cam_num].nROI0_FilterSize[0]);
									dilate(Morph_Out_binary,Out_binary,element_h,Point(-1,-1), BOLT_Param[Cam_num].nROI0_FilterSize[0]);
								}
								J_Delete_Boundary(Out_binary,1);
							}
							...
							if (BOLT_Param[Cam_num].nROI0_MergeFilterSize[0] > 0)
							{
								...
								{
									Mat Morph_Out_binary = Out_binary.clone();
									dilate(Out_binary,Morph_Out_binary,element,Point(-1,-1), BOLT_Param[Cam_num].nROI0_MergeFilterSize[0]);
									erode(Morph_Out_binary,Out_binary,element,Point(-1,-1), BOLT_Param[Cam_num].nROI0_MergeFilterSize[0]);
								}
								else if (BOLT_Param[Cam_num].nROI0_FilterDirection[0] == FILTER_DIRECTION::VERTICAL)
								{
									Mat Morph_Out_binary = Out_binary.clone();
									dilate(Out_binary,Morph_Out_binary,element_v,Point(-1,-1), BOLT_Param[Cam_num].nROI0_MergeFilterSize[0]);
									erode(Morph_Out_binary,Out_binary,element_v,Point(-1,-1), BOLT_Param[Cam_num].nROI0_MergeFilterSize[0]);
								}
								else if (BOLT_Param[Cam_num].nROI0_FilterDirection[0] == FILTER_DIRECTION::HORIZONTAL)
								{
									Mat Morph_Out_binary = Out_binary.clone();
									dilate(Out_binary,Morph_Out_binary,element_h,Point(-1,-1), BOLT_Param[Cam_num].nROI0_MergeFilterSize[0]);
									erode(Morph_Out_binary,Out_binary,element_h,Point(-1,-1), BOLT_Param[Cam_num].nROI0_MergeFilterSize[0]);
								}
								J_Delete_Boundary(Out_binary,1);
							}
							...
							if (contours.size() > 0)
							{
								...
								if (m_max_object_num > -1)
								{
									...
									if(abs(BOLT_Param[Cam_num].Offset_Object_Postion.x) > Out_binary.cols/2 || abs(BOLT_Param[Cam_num].Offset_Object_Postion.y) > Out_binary.rows/2)
									{
										Result_Info[Cam_num] = "";
										for (int ss=1;ss<41;ss++)
										{
							...
							if (BOLT_Param[Cam_num].nTableType == GUIDE_METHOD::CLASS_TYPE || BOLT_Param[Cam_num].nTableType == GUIDE_METHOD::BELT_TYPE) //유리판일때
							{
								if (BOLT_Param[Cam_num].nTableType == GUIDE_METHOD::BELT_TYPE) 
								{
									if (BOLT_Param[Cam_num].nThickness[0] > 0)
									{
										...
										Mat Morph_Out_binary = Out_binary.clone();
										erode(Out_binary,Morph_Out_binary,element_v,Point(-1,-1), t_filter_cnt);
										dilate(Morph_Out_binary,Out_binary,element_v,Point(-1,-1), t_filter_cnt);
										J_Delete_Boundary(Out_binary,1);
									}
								}
...
/*
								...
								{// 유리판 기준 회전 보정
									...
									{
										Mat Out_binary_tmp = BOLT_Param[Cam_num].Gray_Obj_Img.clone();
										blur(BOLT_Param[Cam_num].Gray_Obj_Img, Out_binary_tmp, Size(3,3));
										// Run the edge detector on grayscale
										Canny(Out_binary_tmp, Out_binary, BOLT_Param[Cam_num].nThres_V1[s], BOLT_Param[Cam_num].nThres_V2[s], 3);
									}


									if (BOLT_Param[Cam_num].nROI0_FilterSize[0] > 0)
									{
										Mat Out_binary_tmp = Out_binary.clone();
										erode(Out_binary,Out_binary_tmp,element,Point(-1,-1), BOLT_Param[Cam_num].nROI0_FilterSize[0]);
										dilate(Out_binary_tmp,Out_binary,element,Point(-1,-1), BOLT_Param[Cam_num].nROI0_FilterSize[0]);
										J_Delete_Boundary(Out_binary,1);
									}
									...
									if (t_Left_x >= 0 && t_Left_y > 0 && t_Right_x >= 0 && t_Right_y > 0)
									{
										...
										{
											Mat Out_binary_tmp = Out_binary.clone();
											erode(Out_binary,Out_binary_tmp,element_v,Point(-1,-1),BOLT_Param[Cam_num].nAngleHeightFilterSize[0]);
											dilate(Out_binary_tmp,Out_binary,element_v,Point(-1,-1),BOLT_Param[Cam_num].nAngleHeightFilterSize[0]);
										}
									}
								}
								else 
								...
								Mat Out_binary_tmp = Out_binary.clone();
								erode(Out_binary,Out_binary_tmp,element_v,Point(-1,-1),1);
								dilate(Out_binary_tmp,Out_binary,element_v,Point(-1,-1),1);
								J_Delete_Boundary(Out_binary,1);
								Out_binary_tmp = Out_binary.clone();
								// 못 머리만 찾기
								findContours( Out_binary_tmp, contours, hierarchy, CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE );
								...
								if (m_max_object_num >= 0)
								{
									...
									{// 나사 몸통 기준 회전 보정
										...
										if (BOLT_Param[Cam_num].nAngleHeightFilterSize[0] > 0)
										{
											...
											erode(Out_binary,Out_binary_tmp,element_v,Point(-1,-1),BOLT_Param[Cam_num].nAngleHeightFilterSize[0]);
											dilate(Out_binary_tmp,Out_binary,element_v,Point(-1,-1),BOLT_Param[Cam_num].nAngleHeightFilterSize[0]);
											dilate(Out_binary,Out_binary_tmp,element_v,Point(-1,-1), BOLT_Param[Cam_num].nAngleHeightFilterSize[0]/2);
											erode(Out_binary_tmp,Out_binary,element_v,Point(-1,-1), BOLT_Param[Cam_num].nAngleHeightFilterSize[0]/2);
											J_Delete_Boundary(Out_binary,1);
										}
										...
										{
											fitLine(vec_2center,lines, CV_DIST_L2,1,0.01,0.01);
...
/*
										...
										{
											Out_binary_tmp = BOLT_Param[Cam_num].Gray_Obj_Img.clone();
											blur(BOLT_Param[Cam_num].Gray_Obj_Img, Out_binary_tmp, Size(3,3));
											// Run the edge detector on grayscale
											Canny(Out_binary_tmp, Out_binary, BOLT_Param[Cam_num].nThres_V1[s], BOLT_Param[Cam_num].nThres_V2[s], 3);
										}
							...
							else if (BOLT_Param[Cam_num].nTableType == GUIDE_METHOD::INDEX_TYPE) //Index판일때
							{
								...
								if (BOLT_Param[Cam_num].nAngleHeightFilterSize[0] > 0)
								{
									...
									Mat Out_binary_tmp = t_morph(tt_Sub_ROI).clone();
									dilate(Out_binary_tmp,t_morph(tt_Sub_ROI),element_v,Point(-1,-1), BOLT_Param[Cam_num].nAngleHeightFilterSize[0]);
								}
								...
								erode(t_morph,Out_binary_tmp,element_v,Point(-1,-1), 1);
								dilate(Out_binary_tmp,t_morph,element_v,Point(-1,-1), 1);
								
								//msg.Format("01.bmp");
								//imwrite(msg.GetBuffer(),t_morph);
								...
								if (numOfLables > 0)
								{
									for (int j = 1; j < numOfLables; j++)
									{
										...
										if ((double)(top + height) <=  max(t_Left_y, t_Right_y) && area >= m_Head_value && left > 0 && left+width < t_morph.cols-1)
										{ // 머리부 찾기
											m_Head_value = area;
											m_Head_Idx = j;
										}
										if ((double)(top) >=  min(t_B_Left_y, t_B_Right_y) && area >= m_Body_value && left > 0 && left+width < t_morph.cols-1)
										{ // 바디부 찾기
											m_Body_value = area;
											m_Body_Idx = j;
										}
								...
								Out_binary_tmp = Out_binary(tt_Sub_ROI).clone();
								erode(Out_binary(tt_Sub_ROI),Out_binary_tmp,element_v,Point(-1,-1), 1);
								dilate(Out_binary_tmp,Out_binary(tt_Sub_ROI),element_v,Point(-1,-1), 1);
								//imwrite("02.bmp",Out_binary);
			...
			{// TOP, BOTTOM
				...
				{					
					if (BOLT_Param[Cam_num].nROI0_FilterSize[s] > 0)
					{
						...
						erode(Out_binary,Out_binary_temp,element,Point(-1,-1),BOLT_Param[Cam_num].nROI0_FilterSize[s]);
						dilate(Out_binary_temp,Out_binary,element,Point(-1,-1),BOLT_Param[Cam_num].nROI0_FilterSize[s]);
						//J_Delete_Boundary(Out_binary,1);
					}

					if (BOLT_Param[Cam_num].nCalPreprocessing[s] == 1)
					{
						Mat Edge_Img = Mat::zeros(Out_binary.size(), CV_8UC1);
						Mat Morph_h_Img = Mat::zeros(Out_binary.size(), CV_8UC1);
						Mat Morph_v_Img = Mat::zeros(Out_binary.size(), CV_8UC1);
						Mat Morph_temp_Img = Mat::zeros(Out_binary.size(), CV_8UC1);

						GaussianBlur(Out_binary, Morph_temp_Img, Size(5,5), 3, 3, 0);
						Canny(Morph_temp_Img, Edge_Img, 100, 200, 5, true);

						//imwrite("00.bmp",Edge_Img);


						// 가로방향 선 제거
						erode(Edge_Img,Morph_temp_Img,element_v,Point(-1,-1),2);
						dilate(Morph_temp_Img,Morph_v_Img,element_v,Point(-1,-1),2);
						// 세로방향 선 붙임
						dilate(Morph_v_Img,Morph_temp_Img,element,Point(-1,-1),1);
						erode(Morph_temp_Img,Morph_v_Img,element_v,Point(-1,-1),1);

						// 가로 제거
						erode(Morph_v_Img,Morph_temp_Img,element_v,Point(-1,-1),3);
						dilate(Morph_temp_Img,Morph_v_Img,element_v,Point(-1,-1),9);
						erode(Morph_v_Img,Morph_temp_Img,element_v,Point(-1,-1),6);
						//erode(Morph_temp_Img,Morph_v_Img,element_h,Point(-1,-1),1);
						//dilate(Morph_v_Img,Morph_temp_Img,element_h,Point(-1,-1),1);

						//imwrite("01.bmp",Morph_temp_Img);

						// 세로방향 선 제거
						erode(Edge_Img,Morph_temp_Img,element_h,Point(-1,-1),2);
						dilate(Morph_temp_Img,Morph_h_Img,element_h,Point(-1,-1),2);
						// 가로방향 선 붙임
						dilate(Morph_h_Img,Morph_temp_Img,element,Point(-1,-1),1);
						erode(Morph_temp_Img,Morph_h_Img,element_h,Point(-1,-1),1);

						// 세로 제거
						erode(Morph_h_Img,Morph_temp_Img,element_h,Point(-1,-1),3);
						dilate(Morph_temp_Img,Morph_h_Img,element_h,Point(-1,-1),9);
						erode(Morph_h_Img,Morph_temp_Img,element_h,Point(-1,-1),6);
						//erode(Morph_temp_Img,Morph_h_Img,element_v,Point(-1,-1),1);
						//dilate(Morph_h_Img,Morph_temp_Img,element_v,Point(-1,-1),1);
						//imwrite("02.bmp",Morph_h_Img);

						bitwise_or(Morph_h_Img,Morph_v_Img,Out_binary);
						//imwrite("03.bmp",Morph_temp_Img);
					}
					J_Delete_Boundary(Out_binary,3);
					if (BOLT_Param[Cam_num].nCalCalMethod[s] == 0)
					{//기존 방법으로 양끝 검색으로 계산
						int start_p = -1;
						int end_p = -1;
						int ii = 0;
					...
					else if (BOLT_Param[Cam_num].nCalCalMethod[s] == 1)
					{//BLOBs 끝단으로 측정
						...
						if (contours.size() > 0)
						{
							...
							{  
								...
								//if (tt_rect.y > 0 && tt_rect.y + tt_rect.height < Out_binary.rows-1)
								{
									...
									if (tt_rect.x + tt_rect.width >= t_right)
									{
										t_right = tt_rect.x + tt_rect.width;
									}
							...
							if (t_left != 999999)
							{
								v_Dist = (double)(t_right - t_left)*BOLT_Param[Cam_num].nResolution[0];
								dist_vec.push_back(v_Dist);
								if (m_Text_View[Cam_num] && !ROI_Mode)
								{
									line(Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[s]),Point(t_left,0),Point(t_left,Out_binary.rows),CV_RGB(255,100,0),3);
									line(Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[s]),Point(t_right,0),Point(t_right,Out_binary.rows),CV_RGB(255,100,0),3);
								}
								if (ROI_Mode && ROI_CAM_Num == Cam_num && ROI_Num == s)
								{
									line(Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[s]),Point(t_left,0),Point(t_left,Out_binary.rows),CV_RGB(255,100,0),3);
...
else if (BOLT_Param[Cam_num].nCalCalMethod[s] == 2)
					{//Line fitting으로 계산
						Mat Edge_Img = Mat::zeros(Out_binary.size(), CV_8UC1);
						Mat Temp_Img = Mat::zeros(Out_binary.size(), CV_8UC1);

						GaussianBlur(Out_binary, Temp_Img, Size(5, 5), 3, 3, 0);
						Canny(Temp_Img, Edge_Img, 100, 200, 5, true);

						int count = 0;
						CvPoint* points_Left = new CvPoint[Out_binary.cols];
						CvPoint* points_Right = new CvPoint[Out_binary.cols];
						...
						for(int i=0;i<Out_binary.rows;i++)
						{
							...
							if (start_p != -1 && end_p != -1)
							{
								points_Left[count].x = start_p;points_Left[count].y = ii;
								points_Right[count].x = end_p;points_Right[count].y = ii;
								count++;
							}
							start_p = -1;end_p = -1;
						}
						...
						{
							// 후처리
							Point2d pt1_Left, pt2_Left, ptC_Left,  pt1_Right, pt2_Right, ptC_Right;// 처리 포인트
							Point2d pt_Left_D, pt_Right_D, pt_Temp_D;// 그리기 포인트
							float theta_Left = 0;float theta_Right = 0;
							J_fitLineToPoints( points_Left, count, pt1_Left, pt2_Left, theta_Left );
							J_fitLineToPoints( points_Right, count, pt1_Right, pt2_Right, theta_Right );

							theta_Left *= 180/CV_PI;
							theta_Right *= 180/CV_PI;

							ptC_Left.x = (pt1_Left.x + pt2_Left.x)*0.5;
							ptC_Left.y = (pt1_Left.y + pt2_Left.y)*0.5;pt_Left_D = ptC_Left;
							ptC_Right.x = (pt1_Right.x + pt2_Right.x)*0.5;
							ptC_Right.y = (pt1_Right.y + pt2_Right.y)*0.5;pt_Right_D = ptC_Right;

							// 여기에서 각각 처리
							// 왼쪽 중심에서 오른쪽 직선까지 거리
							float dx_Left = pt2_Left.x - pt1_Left.x;
							float dy_Left = pt2_Left.y - pt1_Left.y;
							float dist_Left = 0;
							if (abs(dx_Left) >= 0.0001)
							{
								dist_Left = fabs((dy_Left * ptC_Right.x - dx_Left * ptC_Right.y - dy_Left*ptC_Left.x + ptC_Left.y*dx_Left)) /(sqrt(dy_Left * dy_Left + dx_Left * dx_Left));
								double t_vv = dist_Left*BOLT_Param[Cam_num].nResolution[0];
								if (t_vv >= BOLT_Param[Cam_num].nCalMinDist[s] && t_vv <= BOLT_Param[Cam_num].nCalMaxDist[s])
								{
									dist_vec.push_back(t_vv);
									float t_AR = (-theta_Left+90.0)*CV_PI/180.0;
									pt_Right_D.x = ptC_Left.x + dist_Left*(1*cos(t_AR) + 0*sin(t_AR));
									pt_Right_D.y = ptC_Left.y + dist_Left*(1*(-sin(t_AR)) + 0*cos(t_AR));
								}
							}
							else
							{
								v_Dist = sqrt((ptC_Left.x - ptC_Right.x)*(ptC_Left.x - ptC_Right.x) + (ptC_Left.y - ptC_Right.y)*(ptC_Left.y - ptC_Right.y))*BOLT_Param[Cam_num].nResolution[0];
								if (v_Dist >= BOLT_Param[Cam_num].nCalMinDist[s] && v_Dist <= BOLT_Param[Cam_num].nCalMaxDist[s])
								{
									dist_vec.push_back(v_Dist);
								}
							...
							// 오른쪽 중심에서 왼쪽 직선까지 거리
							float dx_Right = pt2_Right.x - pt1_Right.x;
							float dy_Right = pt2_Right.y - pt1_Right.y;
							float dist_Right = 0;
							if (abs(dx_Right) >= 0.0001)
							{
								dist_Right = fabs((dy_Right * ptC_Left.x - dx_Right * ptC_Left.y - dy_Right*ptC_Right.x + ptC_Right.y*dx_Right)) /(sqrt(dy_Right * dy_Right + dx_Right * dx_Right));
								double t_vv = dist_Right*BOLT_Param[Cam_num].nResolution[0];
								if (t_vv >= BOLT_Param[Cam_num].nCalMinDist[s] && t_vv <= BOLT_Param[Cam_num].nCalMaxDist[s])
								{
									dist_vec.push_back(t_vv);
									float t_AR = (-theta_Right+90.0)*CV_PI/180.0;
									pt_Left_D.x = ptC_Right.x + dist_Right*(-1*cos(t_AR) + 0*sin(t_AR));
									pt_Left_D.y = ptC_Right.y + dist_Right*(-1*(-sin(t_AR)) + 0*cos(t_AR));
								}
							...
							if (m_Text_View[Cam_num] && !ROI_Mode)
							{
								line(Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[s]), pt1_Left, pt2_Left,CV_RGB(255,100,0),1);
				...
				else if (BOLT_Param[Cam_num].nMethod_Direc[s] == ALGORITHM_TB::H_LENGTH_TB) // Vertical Length
				{
					...
					{//BLOBs 끝단으로 측정
						...
						if (contours.size() > 0)
						{
							...
							{  
								...
								//if (tt_rect.y > 0 && tt_rect.y + tt_rect.height < Out_binary.rows-1)
								{
									if (tt_rect.y <= t_bottom)
									{
										t_bottom = tt_rect.y;
									}
									if (tt_rect.y + tt_rect.height >= t_top)
									{
							...
							if (t_bottom != 999999)
							{
								...
								if (m_Text_View[Cam_num] && !ROI_Mode)
								{
									line(Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[s]),Point(0,t_bottom),Point(Out_binary.cols,t_bottom),CV_RGB(255,100,0),3);
									line(Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[s]),Point(0,t_top),Point(Out_binary.cols,t_top),CV_RGB(255,100,0),3);
								}
								if (ROI_Mode && ROI_CAM_Num == Cam_num && ROI_Num == s)
								{
									line(Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[s]),Point(0,t_bottom),Point(Out_binary.cols,t_bottom),CV_RGB(255,100,0),3);
					...
					{//Line fitting으로 계산
						...
						int count = 0;
						CvPoint* points_Bottom = new CvPoint[Out_binary.cols];
						CvPoint* points_Top = new CvPoint[Out_binary.cols];
						...
						for(int j=0;j<Out_binary.cols;j++)
						{
							...
							if (start_p != -1 && end_p != -1)
							{
								points_Bottom[count].x = jj;points_Bottom[count].y = start_p;
								points_Top[count].x = jj;points_Top[count].y = end_p;
								count++;
							}
							start_p = -1;end_p = -1;
						}
						...
						{
							// 후처리
							Point2d pt1_Bottom, pt2_Bottom, ptC_Bottom,  pt1_Top, pt2_Top, ptC_Top;// 처리 포인트
							Point2d pt_Bottom_D, pt_Top_D, pt_Temp_D;// 그리기 포인트
							float theta_Bottom = 0;float theta_Top = 0;
							J_fitLineToPoints( points_Bottom, count, pt1_Bottom, pt2_Bottom, theta_Bottom );
							J_fitLineToPoints( points_Top, count, pt1_Top, pt2_Top, theta_Top );

							theta_Bottom *= 180/CV_PI;
							theta_Top *= 180/CV_PI;

							ptC_Bottom.x = (pt1_Bottom.x + pt2_Bottom.x)*0.5;
							ptC_Bottom.y = (pt1_Bottom.y + pt2_Bottom.y)*0.5;pt_Bottom_D = ptC_Bottom;
							ptC_Top.x = (pt1_Top.x + pt2_Top.x)*0.5;
							ptC_Top.y = (pt1_Top.y + pt2_Top.y)*0.5;pt_Top_D = ptC_Top;

							// 여기에서 각각 처리
							// 왼쪽 중심에서 오른쪽 직선까지 거리
							float dx_Bottom = pt2_Bottom.x - pt1_Bottom.x;
							float dy_Bottom = pt2_Bottom.y - pt1_Bottom.y;
							float dist_Bottom = 0;
							if (abs(dy_Bottom) >= 0.0001)
							{
								dist_Bottom = fabs((dy_Bottom * ptC_Top.x - dx_Bottom * ptC_Top.y - dy_Bottom*ptC_Bottom.x + ptC_Bottom.y*dx_Bottom)) /(sqrt(dy_Bottom * dy_Bottom + dx_Bottom * dx_Bottom));
								double t_vv = dist_Bottom*BOLT_Param[Cam_num].nResolution[1];
								if (t_vv >= BOLT_Param[Cam_num].nCalMinDist[s] && t_vv <= BOLT_Param[Cam_num].nCalMaxDist[s])
								{
									dist_vec.push_back(t_vv);
									float t_AR = (-theta_Bottom)*CV_PI/180.0;
									pt_Top_D.x = ptC_Bottom.x + dist_Bottom*(0*cos(t_AR) + 1*sin(t_AR));
									pt_Top_D.y = ptC_Bottom.y + dist_Bottom*(0*(-sin(t_AR)) + 1*cos(t_AR));
								}
							}
							else
							{
								v_Dist = sqrt((ptC_Bottom.x - ptC_Top.x)*(ptC_Bottom.x - ptC_Top.x) + (ptC_Bottom.y - ptC_Top.y)*(ptC_Bottom.y - ptC_Top.y))*BOLT_Param[Cam_num].nResolution[1];
								if (v_Dist >= BOLT_Param[Cam_num].nCalMinDist[s] && v_Dist <= BOLT_Param[Cam_num].nCalMaxDist[s])
								{
									dist_vec.push_back(v_Dist);
								}
							...
							// 오른쪽 중심에서 왼쪽 직선까지 거리
							float dx_Top = pt2_Top.x - pt1_Top.x;
							float dy_Top = pt2_Top.y - pt1_Top.y;
							float dist_Top = 0;
							if (abs(dy_Top) >= 0.0001)
							{
								dist_Top = fabs((dy_Top * ptC_Bottom.x - dx_Top * ptC_Bottom.y - dy_Top*ptC_Top.x + ptC_Top.y*dx_Top)) /(sqrt(dy_Top * dy_Top + dx_Top * dx_Top));

								double t_vv = dist_Top*BOLT_Param[Cam_num].nResolution[1];
								if (t_vv >= BOLT_Param[Cam_num].nCalMinDist[s] && t_vv <= BOLT_Param[Cam_num].nCalMaxDist[s])
								{
									dist_vec.push_back(t_vv);
									float t_AR = (-theta_Top)*CV_PI/180.0;
									pt_Bottom_D.x = ptC_Top.x + dist_Top*(0*cos(t_AR) -1*sin(t_AR));
									pt_Bottom_D.y = ptC_Top.y + dist_Top*(0*(-sin(t_AR)) -1*cos(t_AR));
								}
							...
							if (m_Text_View[Cam_num] && !ROI_Mode)
							{
								line(Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[s]), pt1_Bottom, pt2_Bottom,CV_RGB(255,100,0),1);
					...
					if (BOLT_Param[Cam_num].nTableType != GUIDE_METHOD::BELT_TYPE)
					{
						...
						{// 흔들림
							...
							if (contours.size() != 0)
							{
								...
								{
									fitLine(vec_left_center,Left_lines, CV_DIST_L2,1,0.01,0.01);
								...
								{
									fitLine(vec_right_center,Right_lines, CV_DIST_L2,1,0.01,0.01);
								...
								if (BOLT_Param[Cam_num].nOutput[s] == 0)
								{
									if (vec_left_center.size() > 2 && vec_right_center.size() > 2 )
									{
							...
							if (contours.size() != 0)
							{
								...
								for (int i = left;i<left+width;i++)
								{
									for (int j = top;j<top + height;j++)
									{
										if (Out_binary.at<uchar>(j,i) > 0)
										{
											t_Top_Center.x =(float)i;
											t_Top_Center.y = (float)j;
											vec_2center.push_back(t_Top_Center);
											if (ROI_Mode && ROI_CAM_Num == Cam_num && ROI_Num == s)
											{
												circle(Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[s]), t_Top_Center,0,CV_RGB(255,100,0),1);
											}
											if (m_Text_View[Cam_num] && !ROI_Mode)
											{
												circle(Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[s]), t_Top_Center,0,CV_RGB(255,100,0),1);
	...
	if (!Gray_Img[Cam_num].empty())
	{
		if (BOLT_Param[Cam_num].nMethod_Thres[0] == THRES_METHOD::FIRSTROI) // ROI#01 모델사용
		{
			...
			{ // 유레시스 있으면
				...
				m_Cam_Find[Cam_num].SetLightBalance(BOLT_Param[Cam_num].nFindLightBalance[0]);
...
*/
				...
				else if (BOLT_Param[Cam_num].nTableType == GUIDE_METHOD::INDEX_TYPE) //INDEX판일때
				{
					...
					//if (Cam_num == 1)
					//{
					//	imwrite("00_1.bmp",t_morph);
					//}
					//else if (Cam_num == 2)
					//{
					//	imwrite("00_2.bmp",t_morph);
					//}
...
void CImPro_Library::Rotation_Calibration(int Cam_num)
{
	...
	if (!Gray_Img[Cam_num].empty())
	{
		...
		if (BOLT_Param[Cam_num].nROI0_Rotation_Method[s] == ROTATION_METHOD::LEFT)
		{
			...
			if (vec_Point.size() > 6)
			{
				fitLine(vec_Point,lines, CV_DIST_L2,1,0.01,0.01);
		...
		{
			Mat Edge_Image = Mat::zeros(Out_binary.size(), CV_8UC1);
			erode(Out_binary,Edge_Image,element,Point(-1,-1),1);
			subtract(Out_binary,Edge_Image,Edge_Image);
			erode(Edge_Image,Out_binary,element_h,Point(-1,-1),3);
			dilate(Out_binary,Edge_Image,element_h,Point(-1,-1),3);

			double t_cnt = 0;int t_search_range = boundRect.height/4;
			for (int i=boundRect.x + boundRect.width/4;i < boundRect.x+3*boundRect.width/4;i++)
			{
				for (int j=boundRect.y-1;j < boundRect.y+t_search_range;j++)
				{
					if (Edge_Image.at<uchar>(j,i) > 0)
					{
						vec_Point.push_back(Point2f((double)i,(double)j));
						break;
					}
		...
		else if (BOLT_Param[Cam_num].nROI0_Rotation_Method[s] == ROTATION_METHOD::BOTTOM)
		{
			...
			for (int i=boundRect.x+boundRect.width/4;i < boundRect.x+3*boundRect.width/4;i++)
			{
				for (int j=boundRect.y+boundRect.height+1;j > boundRect.y+boundRect.height - t_search_range;j--)
				{
					if (Edge_Image.at<uchar>(j,i) > 0)
					{
						vec_Point.push_back(Point2f((double)i,(double)j));
