	...
	if (!Gray_Img[Cam_num].empty())
	{
		...
		for (int s=0;s<41;s++)
		{
			...
			} else
			{
				...
				{
					tROI.width = BOLT_Param[Cam_num].Gray_Obj_Img.cols - tROI.x -1;
				}
				if (tROI.y + tROI.height >= BOLT_Param[Cam_num].Gray_Obj_Img.rows-1)
				{
					tROI.height = BOLT_Param[Cam_num].Gray_Obj_Img.rows - tROI.y -1;
				}
...
/*
							...
							for (int j = 1; j < numOfLables; j++) 
							{
								...
								{//기준점에서 거리
									...
									if (BOLT_Param[Cam_num].nROI0_BLOB_Min_Size[s] <= (double)area && BOLT_Param[Cam_num].nROI0_BLOB_Max_Size[s] >= (double)area)
									{
										...
										RotatedRect t_Rect =  minAreaRect(contours[0]);
				...
				else if (BOLT_Param[Cam_num].nMethod_Direc[s] == ALGORITHM_TB::CIRCLE_BLOB_SIZE_TB) // Edge Crack
				{
					...
					if (contours.size() > 0)
					{
						for (int i = 0; i < contours.size(); i++) // iterate through each contour.
						{
							if (hierarchy[i][3] == -1)
							{
								int fArea = (int)(contourArea(contours[i]) + 0.5);//t_Circle_Blob_Info.Pixels.size();
								if (BOLT_Param[Cam_num].nROI0_BLOB_Min_Size[s] > (double)fArea || BOLT_Param[Cam_num].nROI0_BLOB_Max_Size[s] < (double)fArea)
								{
					...
					if (BOLT_Param[Cam_num].nTableType != GUIDE_METHOD::BELT_TYPE)
					{
						...
						{// 흔들림
							...
							if (contours.size() != 0)
							{
								...
								//imwrite("00.bmp",Temp_Out_binary);
								boundRect = boundingRect( Mat(contours[t_max_idx]) );
								...
								for (int j = top+height/4;j<=top+2*height/3;j++)
								{
									for (int i = left;i<left+width;i++)
									{
										if (i>=0 && j>=0 && i < Temp_Out_binary.cols && j < Temp_Out_binary.rows)
										{
		...
		/*									msg.Format("%d,%d",i,j);
								...
								if (m_Text_View[Cam_num] && ROI_Mode && ROI_CAM_Num == Cam_num && ROI_Num == s)
								{
									msg.Format("R:%1.3f",abs(Right_angle - Left_angle));
									if (t_left_cnt > 5 && t_right_cnt > 5)
									{
									...
									{
										putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(BOLT_Param[Cam_num].nRect[s].x +left+width/2 - 50,BOLT_Param[Cam_num].nRect[s].y +  top + 3*height/4), FONT_HERSHEY_SIMPLEX, 0.8, CV_RGB(255,255,0), 2, 8);
									}
								}
								if (m_Text_View[Cam_num] && !ROI_Mode)
								{
