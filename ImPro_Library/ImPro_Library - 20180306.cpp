...
void CImPro_Library::J_Rotate(cv::Mat& src, double angle, cv::Mat& dst, int Cam_num)
{
	...
	if (src.channels() == 1)
	{
		cv::warpAffine(src, dst, r, cv::Size(src.cols, src.rows),1,BORDER_TRANSPARENT,CV_RGB(255,255,255));
	} else
	{
		cv::warpAffine(src, dst, r, cv::Size(src.cols, src.rows),1,BORDER_TRANSPARENT,CV_RGB(255,255,255));
	}
...
void CImPro_Library::J_Rotate_Black(cv::Mat& src, double angle, cv::Mat& dst, int Cam_num)
{
	...
	if (src.channels() == 1)
	{
		cv::warpAffine(src, dst, r, cv::Size(src.cols, src.rows),1,BORDER_TRANSPARENT,CV_RGB(0,0,0));
	} else
	{
		cv::warpAffine(src, dst, r, cv::Size(src.cols, src.rows),1,BORDER_TRANSPARENT,CV_RGB(0,0,0));
	}
...
Point2f CImPro_Library::J_Model_Find(int Cam_num)
{
	...
	{// 유레시스 없으면
		...
		//Mat Warp_Img, Diff_Img;
		//AKAZE_Homography(Model_Img[Cam_num],Gray_Img[Cam_num],Warp_Img,Diff_Img);
	...
	{// 유레시스 있으면
		...
		{	
			...
			if (m_Cam_FoundPattern[Cam_num].size() > 0)
			{
				...
				//c_x -= c_x - BOLT_Param[Cam_num].Object_Postion.x;
				//c_y -= c_y - BOLT_Param[Cam_num].Object_Postion.y;
				warpAffine( temp_img, BOLT_Param[Cam_num].Gray_Obj_Img, warp_mat, Gray_Img[Cam_num].size(),1, BORDER_TRANSPARENT,CV_RGB(255,255,255));
	...
	if (!Gray_Img[Cam_num].empty())
	{
		...
		for (int s=0;s<41;s++)
		{
			...
			if (s == 0)
			{
				...
				{
					BOLT_Param[Cam_num].Gray_Obj_Img = Gray_Img[Cam_num].clone();
					continue;
				}
			...
			else if (BOLT_Param[Cam_num].nMethod_Thres[s] == THRES_METHOD::COMPARE) // 비교 차이
			{
				...
				{// 유레시스 없으면
					...
					//Mat Warp_Img, Diff_Img;
					//AfxMessageBox("0");
					//imwrite("00.bmp",Model_Img[Cam_num]);
					//AfxMessageBox("1");
					//imwrite("01.bmp",Gray_Img[Cam_num]);
					//AKAZE_Homography(Model_Img[Cam_num],Gray_Img[Cam_num],Warp_Img,Diff_Img);
					//Mat Color_img;
					//AfxMessageBox("2");
					//imwrite("02.bmp",Diff_Img);

					//cvtColor(Diff_Img,Dst_Img[Cam_num](comp_rect),CV_GRAY2BGR);

					//Mat dark_img = Mat::zeros(comp_rect.size(), CV_8UC1);
					//Mat bright_img = Mat::zeros(comp_rect.size(), CV_8UC1);
					//threshold(Diff_Img,dark_img,128.0-BOLT_Param[Cam_num].nThres_V1[s],255,CV_THRESH_BINARY_INV);
					//threshold(Diff_Img,bright_img,128.0+BOLT_Param[Cam_num].nThres_V2[s],255,CV_THRESH_BINARY);
			...
			{// TOP, BOTTOM
				...
				else if (BOLT_Param[Cam_num].nMethod_Direc[s] == ALGORITHM_TB::AREA_BLOB_TB) // BLOB Size
				{
					if (BOLT_Param[Cam_num].nDirecFilterUsage[s] == 0 || BOLT_Param[Cam_num].nDirecFilterUsage[s] == 2)
					{
						...
						int numOfLables = connectedComponentsWithStats(Out_binary, label,   
							stats, centroids, 8,CV_32S);
						int t_cnt = 0;int area = 0;int t_w = 0;int t_h = 0;int t_x = 0;int t_y = 0;
						if (numOfLables > 200)
						{
							numOfLables = 200;
						}
				...
				else if (BOLT_Param[Cam_num].nMethod_Direc[s] == ALGORITHM_TB::CIRCLE_BLOB_SIZE_TB) // Edge Crack
				{
					...
					if (BOLT_Param[Cam_num].nCircle1Thickness[s] > 0 && BOLT_Param[Cam_num].nCircle1Radius[s] >= 0)
					{
						//ellipse(Mask,P_Center,Size(BOLT_Param[Cam_num].nCircle1Radius[s],BOLT_Param[Cam_num].nCircle1Radius[s]),0,BOLT_Param[Cam_num].nCircleStartAngle[s],BOLT_Param[Cam_num].nCircleEndAngle[s],Scalar(255,255,255),BOLT_Param[Cam_num].nCircle1Thickness[s],LINE_AA,0);
					...
					int Contour_cnt = countNonZero(Out_binary);;
					//imwrite("00.bmp",Out_binary);
					//AfxMessageBox("1");
					...
					//imwrite("00.bmp",Out_binary);
					if (numOfLables >= 1 && Contour_cnt > 0)
					{
						//if (BOLT_Param[Cam_num].nCircleOutputMethod[s] >= 2 && BOLT_Param[Cam_num].nCircleOutputMethod[s] <= 6)
						//높이,넓이,축각도
						//"계산방법(0:픽셀수,1:갯수,2:높이,3:넓이,4:축각도,5:각도(중심에서),6:사이각,7:픽셀수(가장자리 제외),8:픽셀수(가장자리))";
						findContours( Out_binary.clone(), contours, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE );
						vector<RotatedRect> minRect(contours.size());
						vector<Circle_Blob_Info> vec_Circle_Blob_Info(contours.size());
						// 각도 계산
						vector<double> vX;
						vector<double> vY;
						//msg.Format("%d/%d",contours.size(), numOfLables);
						//AfxMessageBox(msg);
						//#pragma omp parallel for
						for(int i=0; i<contours.size(); i++)
						{
							minRect[i] = minAreaRect(contours[i]);
							vec_Circle_Blob_Info[i].centerPoint = minRect[i].center;
							vec_Circle_Blob_Info[i].minRect = minRect[i];
							vec_Circle_Blob_Info[i].fWidth = minRect[i].size.width;
							vec_Circle_Blob_Info[i].fHeight = minRect[i].size.height;
							vec_Circle_Blob_Info[i].fAngle = minRect[i].angle;
							vX.push_back(minRect[i].center.x - P_Center.x);
							vY.push_back(minRect[i].center.y - P_Center.y);
							vec_Circle_Blob_Info[i].contour = contours[i];
							vec_Circle_Blob_Info[i].fLabelNum = -1;
							vec_Circle_Blob_Info[i].fBoundaryHitNum = 0;
						}
						...
						{					
							...
							vec_Circle_Blob_Info[i].fBoundaryHitNum = -1;
						}
						//continue;
						...
						for(int i=0; i<contours.size(); i++)
						{
							...
							for (int j = 1; j < numOfLables; j++) 
							{
								...
								if (vec_Circle_Blob_Info[i].fLabelNum == -1)
								{
									for (int y = top; y < bottom; ++y) {
										int *tlabel = label.ptr<int>(y);
										for (int x = left; x < right; ++x) {
											if (tlabel[x] == j)
											{
												if (vec_Circle_Blob_Info[i].fLabelNum == -1)
												{
													if (pointPolygonTest( vec_Circle_Blob_Info[i].contour, Point(x,y), false ) == 1)
													{
														vec_Circle_Blob_Info[i].fLabelNum = j;
														vec_Circle_Blob_Info[i].fArea = area;
														//msg.Format("%d/%d",area,i);
														//AfxMessageBox(msg);
													}
												}
												else
							...
							if (vec_Circle_Blob_Info[i].fLabelNum > -1) 
							{
								left = stats.at<int>(vec_Circle_Blob_Info[i].fLabelNum, CC_STAT_LEFT);
								top = stats.at<int>(vec_Circle_Blob_Info[i].fLabelNum, CC_STAT_TOP);
								right = left + stats.at<int>(vec_Circle_Blob_Info[i].fLabelNum, CC_STAT_WIDTH);
								bottom = top + stats.at<int>(vec_Circle_Blob_Info[i].fLabelNum, CC_STAT_HEIGHT);
								...
								//#pragma omp parallel for
								if (vec_Circle_Blob_Info[i].fBoundaryHitNum == -1)
								{
									if (BOLT_Param[Cam_num].nNumConnectedCircleBLOB[s] >= 1)
									{
										for (int y = top; y <= bottom; y++) 
										{
											int *tlabel = label.ptr<int>(y);
											...
											for (int x = left; x <= right; x++) 
											{
												if (tlabel[x] == vec_Circle_Blob_Info[i].fLabelNum)
												{
													t_dist = sqrt(((float)x-P_Center.x)*((float)x-P_Center.x)+((float)y-P_Center.y)*((float)y-P_Center.y));
											...
											if (t_Circle1_connect_cnt >= 2 || t_Circle2_connect_cnt >= 2)
											{
												break;
											}
										}
										vec_Circle_Blob_Info[i].fBoundaryHitNum = max(t_Circle1_connect_cnt,t_Circle2_connect_cnt);
									}
								}
							}
						}
							...
							if (BOLT_Param[Cam_num].nROI0_BLOB_Min_Size[s] <= (double)vec_Circle_Blob_Info[i].fArea && BOLT_Param[Cam_num].nROI0_BLOB_Max_Size[s] >= (double)vec_Circle_Blob_Info[i].fArea)
							{
											...
											if (m_Text_View[Cam_num] && !ROI_Mode)
											{
												for (int yy = top; yy < bottom; ++yy) {
													...
													for (int xx = left; xx < right; ++xx) {
														if (tlabel[xx] == vec_Circle_Blob_Info[i].fLabelNum)
														{
															pixel[xx][2] = 255;  
															pixel[xx][1] = 0;
															pixel[xx][0] = 0;
								...
								{ // 바운더리 걸리는거 없을때
									...
									{//"계산방법(0:픽셀수,1:갯수,2:높이,3:넓이,4:축각도,5:각도(중심에서),6:사이각,7:픽셀수(가장자리 제외),8:픽셀수(가장자리))";
										...
										if (m_Text_View[Cam_num] && ROI_Mode && ROI_CAM_Num == Cam_num && ROI_Num == s)
										{
											//msg.Format("%d/%d",i,vec_Circle_Blob_Info[i].fLabelNum);
											//AfxMessageBox(msg);
											double x = vec_Circle_Blob_Info[i].centerPoint.x;
											double y = vec_Circle_Blob_Info[i].centerPoint.y;
											//#pragma omp parallel for
									...
									{//"계산방법(0:픽셀수,1:갯수,2:높이,3:넓이,4:축각도,5:각도(중심에서),6:사이각,7:픽셀수(가장자리 제외),8:픽셀수(가장자리))";
										...
										if (m_Text_View[Cam_num] && ROI_Mode && ROI_CAM_Num == Cam_num && ROI_Num == s)
										{
											...
											putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(BOLT_Param[Cam_num].nRect[s].x + BOLT_Param[Cam_num].nRect[s].width+3,BOLT_Param[Cam_num].nRect[s].y + 12*t_cnt), FONT_HERSHEY_SIMPLEX, fontScale, CV_RGB(255,100,0), 1, 8);
											msg.Format("%1.1f",t_temp);
											putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(BOLT_Param[Cam_num].nRect[s].x+(int)((x1+x2)/2),BOLT_Param[Cam_num].nRect[s].y+(int)((y1+y2)/2)), fontFace, fontScale, CV_RGB(0,255,0), 1, 8);
										}
										t_cnt++;
									}
									else if (BOLT_Param[Cam_num].nCircleOutputMethod[s] == 7)
					...
					//					if (BOLT_Param[Cam_num].nCircleOutputMethod[s] == 0)
					//					{
					//						dist_vec.push_back((double)area);
					//						if (m_Text_View[Cam_num] && !ROI_Mode)
					//						{
					...
					//					t_cnt++;
					//				}
					//				if (BOLT_Param[Cam_num].nCircleOutputMethod[s] == 2)
					//				{//높이
	...
	if (!Gray_Img[Cam_num].empty())
	{
		if (BOLT_Param[Cam_num].nMethod_Thres[0] == THRES_METHOD::FIRSTROI) // ROI#01 모델사용
		{
			...
			{ // 유레시스 있으면
				...
				//AfxMessageBox("4");
				m_Cam_Find[Cam_num].SetScaleTolerance(0.00f);
				m_Cam_Find[Cam_num].SetAngleBias(0.00f);
				m_Cam_Find[Cam_num].SetAngleTolerance(180.00f);
				//m_Cam_Find[Cam_num].SetFindExtension(100);
				...
				if (m_min_object_num > -1)
				{
					...
					if (ROI_Mode && ROI_CAM_Num == Cam_num && ROI_Num == s)
					{
						...
						for (int y = 0; y < Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[s]).rows; ++y) {
							...
							Vec3b* pixel = Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[s]).ptr<Vec3b>(y);
							for (int x = 0; x < Dst_Img[Cam_num](BOLT_Param[Cam_num].nRect[s]).cols; ++x) {
								if (tlabel[x] == m_min_object_num)
								{
									pixel[x][2] = 0;  
									pixel[x][1] = 255;
									pixel[x][0] = 0;
								}
								else if (tlabel[x] != m_min_object_num && tlabel[x] > 0)
								{
									//pixel[x][2] = 0;  
									//pixel[x][1] = 0;
									pixel[x][0] = 0;
