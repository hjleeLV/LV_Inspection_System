...
int GeoMatch::CreateGeoMatchModel(Mat templateArr, double maxContrast, double minContrast)
{
	...
	CvSize t_CvSize; t_CvSize.width = Ssize.width; t_CvSize.height = Ssize.height;
	double **magMat;
	CreateDoubleMatrix(magMat, t_CvSize);
	...
	// save selected edge information
			...
			if (flag != 0)
			{
				if (fdx != 0 || fdy != 0)
				{
					...
					cordinates[(int)noOfCordinates].x = (int)curX;
					cordinates[(int)noOfCordinates].y = (int)curY;
					edgeDerivativeX[(int)noOfCordinates] = fdx;
					edgeDerivativeY[(int)noOfCordinates] = fdy;

					//handle divide by zero
					if (MagG != 0)
						edgeMagnitude[(int)noOfCordinates] = 1 / MagG;  // gradient magnitude 
					else
						edgeMagnitude[(int)noOfCordinates] = 0;
	...
	for (int m = 0; m<(int)(noOfCordinates-0.5f); m++)
	{
		int temp;

		temp = cordinates[m].x;
		cordinates[m].x = temp - (int)centerOfGravity.x;
		temp = cordinates[m].y;
		cordinates[m].y = temp - (int)centerOfGravity.y;
	}
...
double GeoMatch::FindGeoMatchModel(Mat srcarr, double minScore, double greediness, Point2f *resultPoint)
{
	...
	for (i = 0; i < Ssize.height-1; i++)
	{
		for (j = 0; j < Ssize.width-1; j++)
		{
	...
	for (i = 0; i < Ssize.height; i++)
	{
		for (j = 0; j < Ssize.width; j++)
		{
			...
			if (partialScore > resultScore)
			{
				...
				resultPoint->x = (float)i;			// result coordinate X		
				resultPoint->y = (float)j;			// result coordinate Y
			}
		}
	}
	AfxMessageBox(L"1-2");
