...
enum THRES_METHOD
{
	...
	DIFFAVG,					//8
	COMPARE,					//9
	COLOR_CH1_BINARY_BETWEEN,	//10
	COLOR_CH2_BINARY_BETWEEN,	//11
};
...
enum ALGORITHM_S
{
	...
	BEVELLING_MEASUREMENT_S,			//16:Bevelling Measurement
	AI_INSPECTION_S,					//17:AI Inspection
	SSF_S,								//18:SSF	
};
...
struct Point3D { float DIST, CX, CY;};
...
struct AI_Model // AI 추가
{
	Net net[41];
	CString model[41];
	bool model_loaded[41];
	CString model_Old[41];
	AI_Model()
	{
		for (int i = 0; i < 41; i++)
		{
			model[i] = L"";
			model_Old[i] = L"";
			model_loaded[i] = false;
		}
	}
};

struct BLOB_Features {
	int Label_No;							// Label 번호
	Rect MBR;								// ROI 정보
	RotatedRect minRect;					// MinRect 정보
	float GD;								// Gray difference
...
public:
	...
	void Set_Image(unsigned char* src_u8, long size_x, long size_y, long channel, int Cam_num);	// 카메라 이미지 입력
	...
	void Set_Mask_Image(unsigned char* src_u8, long size_x, long size_y, long channel, int Cam_num, int ROI_Idx);	// Mask 이미지 입력
	...
	double fontScale;
	int thickness;
	bool Alg_Run_Check[4];

	CString Model_Name;
	CString Model_Save_Folder;
	bool AI_Image_Save;
	CString SSF_Save_Folder;
	int SSF_Save_Format; //0:BMP,1:JPG,2:PNG
	bool SSF_Image_Save[4];
	CString model_save;
	bool model_save_flag;
	...
	void J_TemplateMatch(Mat graySourceImage, Mat grayTemplateImage, int Cam_num);
	bool J_Intersection(Point2f o1, Point2f p1, Point2f o2, Point2f p2, Point2f& r);
	bool EasyFind_Check();
	...
	bool J_Model_Find_Recursive_check;
	Point2f J_Model_Find(int Cam_num);
	float Cal_Angle(Mat& Out_binary, int Cam_num);
	void J_fitLineToPoints( CvPoint* points, int num_points, Point2d & pt1, Point2d & pt2, float & theta );
	...
	int t_Alg_Run_Check_retun_cnt[4];
	bool ROI_Mode;
	int ROI_Num;
	int ROI_CAM_Num;

	CString Offset_Object_Postion[4];			// 검사대상 ROI 검사시 설정대비 LT좌표 움직인 량;

	AI_Model AI_Model[4]; // AI 추가
	void AI_Model_Load(int Cam_Num, int ROI_Idx); // AI 추가
	CString AI_Inspect(Mat& Img, int Cam_Num, int ROI_Idx, int& Result_ID, float& Result_Confidence); // AI 추가
	...
	struct ALG_BOLT_ParameterData {
		...
		int nCalPreprocessing[41];				// Edge 전처리(0:미사용, 1:사용)
		int nCalCalMethod[41];					// 계산방법(0:기존대로, 1:BLOB기준 끝단)
		...
		int nROI0_BLOBSelect[41];				// ROI0의 BLOB 선택 방법
		int nROI0_ColorPreprocessing[41];		// ROI0의 COLOR 전처리

		double nDiameter_Min_Size[41];			// 직경 최소 길이
		double nDiameter_Max_Size[41];			// 직경 최대 길이			
		int nDiameter_Method[41];				// 직경 계산 옵션
		int nDiameter_Direction[41];			// 직경 계산 방향 옵션
		...
		int nAngleCalMethod[41];				// 회전 계산 방법
		int nSRotationCalMethod[41];			// 특수 회전 계산 방법
		int nSRotationTopHeight[41];			// 특수 회전 상부 높이
		...
		int nCrossMethod[41];					// 계산 방법(0:모두,1:좌상에서 우하,2:좌하에서 우상)
		...
		int nErodeFilterCNT[41];				// ERODE Filter 횟수
		int nDilateFilterCNT[41];				// DILATE Filter 횟수
		int nMPMethod[41];						// Filter 처리방법
		...
		float nFindScaleTolerance[41];
		float nFindAngleTolerance[41];
		int nFindFindExtension[41];
		float nFindLightBalance[41];
		int  nFindPostProcessing[41];
		float nFindScore[41];
		int  nFindTopMargin[41];
		int  nFindBottomMargin[41];
		...
		int nBendingOutput[41];					// 몸통휨 출력 옵션(0:차이거리,1:각도)
		Point2f nSideBottomLeft;
		Point2f nSideBottomRight;

		float nSSFXLifting[41];
		float nSSFYLifting[41];
		int nSSFDefectFilterX[41];
		int nSSFDefectFilterY[41];
		int nSSFBaseFilterX[41];
		int nSSFBaseFilterY[41];
		int nSSFOpened[41];
		int nSSFClosed[41];
		float nSSFGDDark[41];
		...
		int nSSFOutput[41];
		int nSSFAIClass[41];
		double nSSFResult[41];
		Mat nSSFOmitImage;
		vector<BLOB_Features> vecSSF_BLOB;
		...
		ALG_BOLT_ParameterData()
		{
			...
			for (int i=0;i<41;i++)
			{
				...
				nColorMethod[i] = nColorMinThres[i] = nColorMaxThres[i] = nColorOutput[i] = nBlurFilterCNT[i] = nROI0_MergeFilterSize[i] = nROI0_BLOBSelect[i] = 0;
				nFindScaleTolerance[i] = 0.05f;
				nFindAngleTolerance[i] = 360.0f;
				nFindFindExtension[i] = nFindPostProcessing[i] = 0;
				nFindLightBalance[i] = 0.0f;
				nBendingOutput[i] = 0;
				nDiameter_Method[i] = 0;
				nSRotationCalMethod[i] = 0;
				nSRotationTopHeight[i] = 0;
			}
