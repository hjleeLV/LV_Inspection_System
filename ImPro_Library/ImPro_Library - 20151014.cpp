...
bool CImPro_Library::RUN_Algorithm_CAM1()
{
	...
	if (!Gray_Img[Cam_num].empty())
	{
		...
		threshold(Outter_Circle,Inner_Circle,ALG_C1_Param.nInnerThreshold,255,CV_THRESH_BINARY);
		...
		{  
				...
				boundRect.y > 1 || boundRect.y + boundRect.height < Gray_Img[Cam_num].rows-1 )
			{
				...
				if (Result_Text_View)
				{
					drawContours(Dst_Img[Cam_num], contours, k, CV_RGB(255,100,0), 2, 1, hierarchy);
				}
		...
		if (Result_Text_View)
		{
			...
			putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(circle_info[0]-40,circle_info[1] - 10), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(255,100,0), 2, 8);
		}
		//Rect Rect_Contour = boundingRect(contours[m_max_object_num]);
		if (m_area != 0)
		{
