...
bool CImPro_Library::RUN_Algorithm_CAM0()
{
	...
	if (!Gray_Img[Cam_num].empty())
	{
		...
		if (t_thres > 220)
		{
			Result_Info[Cam_num].Format("C0:00_-1_C0:01_-1_C0:02_-1_C0:03_-1_C0:04_-1");
		...
		Rect_Contour.height -= ALG_C0_Param.nBottomMargin;
		
		// MBR 이 영상의 하단과 상단에 붙을 경우 에러 처리함.
		if(Rect_Contour.x <= 1 || Rect_Contour.x + Rect_Contour.width >= Gray_Img[Cam_num].cols - 1 || 
			Rect_Contour.y <= 1 || Rect_Contour.y + Rect_Contour.height > Gray_Img[Cam_num].rows )
		{
			Result_Info[Cam_num].Format("C0:00_-1_C0:01_-1_C0:02_-1_C0:03_-1_C0:04_-1");
			rectangle(Dst_Img[Cam_num],Rect(100,100,Out_binary.cols-200,Out_binary.rows-200),Scalar(0,0,255),2);
			return true;
		...
		if (Result_Text_View)
		{
			...
			msg.Format("W(%1.3f)",ALG_C0_Param.oTopDistance*ALG_C0_Param.Res_X + ALG_C0_Param.Offset[0]);
		...
		if (Result_Text_View)
		{
			...
			//circle(Dst_Img[Cam_num], Point((int)ALG_C0_Param.oTopCenterX,Rect_Contour.y+ALG_C0_Param.nTopMargin+ALG_C0_Param.nTopHeight/2),2,CV_RGB(255,255,0),3);
			//circle(Dst_Img[Cam_num], Point((int)ALG_C0_Param.oTopCenterX,Rect_Contour.y+ALG_C0_Param.nTopMargin+ALG_C0_Param.nTopHeight/2),1,CV_RGB(255,0,0),1);
			msg.Format("LH0(%1.3f)",ALG_C0_Param.oLeftDistance*ALG_C0_Param.Res_Y + ALG_C0_Param.Offset[3]);
			...
			msg.Format("LH1(%1.3f)",v_dist_Left_Top_Edge*ALG_C0_Param.Res_Y + ALG_C0_Param.Offset[1]);
			...
			msg.Format("LH2(%1.3f)",v_dist_Left_Edge_Bottom*ALG_C0_Param.Res_Y + ALG_C0_Param.Offset[2]);
		...
		if (Result_Text_View)
		{
			...
			//circle(Dst_Img[Cam_num], Point((int)ALG_C0_Param.oTopCenterX,Rect_Contour.y+ALG_C0_Param.nTopMargin+ALG_C0_Param.nTopHeight/2),2,CV_RGB(255,255,0),3);
			//circle(Dst_Img[Cam_num], Point((int)ALG_C0_Param.oTopCenterX,Rect_Contour.y+ALG_C0_Param.nTopMargin+ALG_C0_Param.nTopHeight/2),1,CV_RGB(255,0,0),1);
			msg.Format("RH0(%1.3f)",ALG_C0_Param.oRightDistance*ALG_C0_Param.Res_Y + ALG_C0_Param.Offset[3]);
			...
			msg.Format("RH1(%1.3f)",v_dist_Right_Top_Edge*ALG_C0_Param.Res_Y + ALG_C0_Param.Offset[1]);
			...
			msg.Format("RH2(%1.3f)",v_dist_Right_Edge_Bottom*ALG_C0_Param.Res_Y + ALG_C0_Param.Offset[2]);
			...
			msg.Format("H0(%1.3f)", ALG_C0_Param.Offset[3] + ALG_C0_Param.Res_Y*(ALG_C0_Param.oLeftDistance+ALG_C0_Param.oRightDistance)/2.0);			
			...
			msg.Format("H1(%1.3f)",ALG_C0_Param.Offset[1] + ALG_C0_Param.Res_Y*(v_dist_Left_Top_Edge+v_dist_Right_Top_Edge)/2.0);
			...
			msg.Format("H2(%1.3f)",ALG_C0_Param.Offset[2] + ALG_C0_Param.Res_Y*(v_dist_Left_Edge_Bottom+v_dist_Right_Edge_Bottom)/2.0);
		...
		Mat Top_Defect,Top_Defect_Mor;
		threshold(Gray_Img[Cam_num],Top_Defect,ALG_C0_Param.nEdgeThreshold,255,CV_THRESH_BINARY_INV);
		
		erode(Top_Defect,Top_Defect_Mor,element_v,Point(-1,-1),5);
		dilate(Top_Defect_Mor,Top_Defect_Mor,element_v,Point(-1,-1),5);
		subtract(Top_Defect,Top_Defect_Mor,Top_Defect_Mor);
		Mat element_hh = getStructuringElement(MORPH_RECT, Size(3, 1), Point(-1, -1) );
		erode(Top_Defect_Mor,Top_Defect_Mor,element_hh,Point(-1,-1),1);
		dilate(Top_Defect_Mor,Top_Defect_Mor,element_hh,Point(-1,-1),1);
		//imwrite("00.bmp",Top_Defect_Mor);

		vector<vector<Point> > topcontours;
		vector<Vec4i> tophierarchy;

		findContours( Top_Defect_Mor.clone(), topcontours, tophierarchy, CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE );
		m_max_object_num = -1;m_max_object_value = 0;int t_max = 0;
		for( int k = 0; k < topcontours.size(); k++ )
		{  
			boundRect = boundingRect( Mat(topcontours[k]) );
			if (m_max_object_value<= boundRect.width
				&& boundRect.height > 2
				&& boundRect.y >= ALG_C0_Param.nTopYforMaxDist 
				&& boundRect.y <= ALG_C0_Param.nBottomYforMaxDist
				&& (boundRect.x <= Rect_Contour.x + Rect_Contour.width/5
				|| boundRect.x >= Rect_Contour.x + 4*Rect_Contour.width/5))
			{
				m_max_object_value = boundRect.width;
				m_max_object_num = k;
			}
		}
		if (m_max_object_num != -1)
		{
			boundRect = boundingRect( Mat(topcontours[m_max_object_num]) );
			t_max = boundRect.width;
		}

		if (Result_Text_View)
		{
			line( Dst_Img[Cam_num], Point(0,ALG_C0_Param.nTopYforMaxDist), Point(Dst_Img[Cam_num].cols,ALG_C0_Param.nTopYforMaxDist), CV_RGB(0,255,255), 1, CV_AA);
			line( Dst_Img[Cam_num], Point(0,ALG_C0_Param.nBottomYforMaxDist), Point(Dst_Img[Cam_num].cols,ALG_C0_Param.nBottomYforMaxDist), CV_RGB(0,255,255), 1, CV_AA);

			if (m_max_object_num != -1)
			{
				rectangle(Dst_Img[Cam_num],boundRect, CV_RGB(255,0,0), 2, CV_AA);
				//J_DrawArrow(Dst_Img[Cam_num], Point(nVec_MinMax_Left[t_max_idx],ALG_C0_Param.nTopYforMaxDist + t_max_idx), Point(nVec_MinMax_Right[t_max_idx],ALG_C0_Param.nTopYforMaxDist + t_max_idx), CV_RGB(0,255,255), 10 , 2, CV_AA, 0);
				//J_DrawArrow(Dst_Img[Cam_num], Point(nVec_MinMax_Right[t_max_idx],ALG_C0_Param.nTopYforMaxDist + t_max_idx), Point(nVec_MinMax_Left[t_max_idx],ALG_C0_Param.nTopYforMaxDist + t_max_idx), CV_RGB(0,255,255), 10 , 2, CV_AA, 0);
				msg.Format("DW(%1.3f)",ALG_C0_Param.Res_X*(double)t_max + ALG_C0_Param.Offset[5]);
				putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(boundRect.x,boundRect.height+boundRect.y+20), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(255,0,0), 2, 8);
			}
		}


		//for(int i=ALG_C0_Param.nTopYforMaxDist; i< ALG_C0_Param.nBottomYforMaxDist;i++)
		//{
		//	if (i<0 || i >= Gray_Img[Cam_num].rows)
		//	{
		//		continue;
		...
		//	nLeft = -1;
		//	for(int j=50; j<Gray_Img[Cam_num].cols/2 ; j++)
		//	{
		//		if (j<0 || j >= Gray_Img[Cam_num].cols)
		//		{
		//			continue;
		//		}
		//		if(Gray_Img[Cam_num].at<uchar>(i,j)<=ALG_C0_Param.nEdgeThreshold)
		//		{
		//			nLeft = j;
		//			break;
		//		}
		//	}
		//	nRight = -1;
		//	for(int j=Gray_Img[Cam_num].cols-50; j>Gray_Img[Cam_num].cols/2 ; j--)
		//	{
		//		if (j<0 || j >= Gray_Img[Cam_num].cols)
		//		{
		//			continue;
		...
		//	} else
		//	{
		//		nVec_MinMax_Left.push_back(0);
		//		nVec_MinMax_Right.push_back(0);
		//		nVec_MinMaxDist_Right.push_back(0);
		//	}
		//}
		...
		//		J_DrawArrow(Dst_Img[Cam_num], Point(nVec_MinMax_Left[t_max_idx],ALG_C0_Param.nTopYforMaxDist + t_max_idx), Point(nVec_MinMax_Right[t_max_idx],ALG_C0_Param.nTopYforMaxDist + t_max_idx), CV_RGB(0,255,255), 10 , 2, CV_AA, 0);
		//		J_DrawArrow(Dst_Img[Cam_num], Point(nVec_MinMax_Right[t_max_idx],ALG_C0_Param.nTopYforMaxDist + t_max_idx), Point(nVec_MinMax_Left[t_max_idx],ALG_C0_Param.nTopYforMaxDist + t_max_idx), CV_RGB(0,255,255), 10 , 2, CV_AA, 0);
		//		msg.Format("Max(%1.3f)",ALG_C0_Param.Res_X*(double)t_max + ALG_C0_Param.Offset[5]);
		//		putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(Gray_Img[Cam_num].cols/2-200,-10+ALG_C0_Param.nTopYforMaxDist + t_max_idx), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(0,255,255), 2, 8);
		//	}
		//}

		Result_Info[Cam_num].Format("C0:00_%1.4f_C0:01_%1.4f_C0:02_%1.4f_C0:03_%1.4f_C0:04_%1.4f"
			,ALG_C0_Param.oTopDistance*ALG_C0_Param.Res_X + ALG_C0_Param.Offset[0]
		    ,ALG_C0_Param.Offset[1]+ALG_C0_Param.Res_Y*(v_dist_Left_Top_Edge+v_dist_Right_Top_Edge)/2.0
			,ALG_C0_Param.Offset[2]+ALG_C0_Param.Res_Y*(v_dist_Left_Edge_Bottom+v_dist_Right_Edge_Bottom)/2.0
			,ALG_C0_Param.Offset[3]+ALG_C0_Param.Res_Y*(ALG_C0_Param.oLeftDistance+ALG_C0_Param.oRightDistance)/2.0
			,ALG_C0_Param.Res_X*(double)t_max + ALG_C0_Param.Offset[4]);
	} else
	{
		Result_Info[Cam_num].Format("C0:00_-1_C0:01_-1_C0:02_-1_C0:03_-1_C0:04_-1");
	}
...
bool CImPro_Library::RUN_Algorithm_CAM1()
{
	...
	if (!Gray_Img[Cam_num].empty())
	{
		...
		double t_thres = threshold(CP_Gray_Img,Out_binary,ALG_C1_Param.nBGThreshold,255,CV_THRESH_BINARY);
		Mat Thres_Out_binary = Out_binary.clone();
		...
		J_Delete_Boundary(Out_binary,5);
		...
		Mat Out_Ring = Outter_Circle.clone();

		J_Fill_Hole(Outter_Circle);
		//dilate(Outter_Circle,Outter_Circle,element,Point(-1,-1),2);
		erode(Outter_Circle,Outter_Circle,element,Point(-1,-1),30);
		dilate(Outter_Circle,Outter_Circle,element,Point(-1,-1),31);
		...
		double Gagong_Diameter = (circle_info[2] + circle_info[3])/2;
		if (Result_Text_View)
		{
			ellipse(Dst_Img[Cam_num],Point((int)circle_info[0],(int)circle_info[1]),Size(circle_info[2]/2,circle_info[3]/2),circle_info[4],0,360,CV_RGB(0,255,0),2,1,0);
			circle(Dst_Img[Cam_num],Point((int)circle_info[0],(int)circle_info[1]),2,CV_RGB(0,255,0),3);
			msg.Format("D(%1.3f)",2*Gagong_Diameter*ALG_C1_Param.Res_X + ALG_C1_Param.Offset[0]);
			putText(Dst_Img[Cam_num], msg.GetBuffer(), Point((int)circle_info[0]-50,(int)circle_info[1]-circle_info[3]/2 - 10), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(255,100,0), 2, 8);
		}

		findContours( Out_Ring.clone(), contours, hierarchy, CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE );
		...
		Out_Ring = Mat::zeros(Out_Ring.size(), CV_8UC1);
		drawContours(Out_Ring, contours, m_max_object_num, CV_RGB(255,255,255), CV_FILLED, 8, hierarchy);
		
		threshold(CP_Gray_Img,Thres_Out_binary,ALG_C1_Param.nInnerThreshold,255,CV_THRESH_BINARY);
		//imwrite("01.bmp",Thres_Out_binary);
		multiply(Outter_Circle,Thres_Out_binary,Outter_Circle);
		//imwrite("01.bmp",Outter_Circle);

		//bitwise_and(Outter_Circle,Thres_Out_binary,Outter_Circle);
		dilate(Out_Ring,Out_Ring,element,Point(-1,-1),8);
		//imwrite("01.bmp",Outter_Circle);
		Mat Band_img;
		subtract(Outter_Circle,Out_Ring,Band_img);
		//imwrite("03.bmp",Band_img);


		Mat Filled_Outter_Circle = Out_Ring.clone();

		//J_Fill_Hole(Filled_Outter_Circle);
		//subtract(Filled_Outter_Circle,Outter_Circle,Filled_Outter_Circle);
		//imwrite("03.bmp",Filled_Outter_Circle);


		//Mat Band_img;
		//dilate(Filled_Outter_Circle,Outter_Circle,element,Point(-1,-1),20);
		//erode(Outter_Circle,Outter_Circle,element,Point(-1,-1),20);
		//subtract(Outter_Circle,Filled_Outter_Circle,Band_img);


		erode(Band_img,Band_img,element,Point(-1,-1),1);
		dilate(Band_img,Band_img,element,Point(-1,-1),5);
		erode(Band_img,Band_img,element,Point(-1,-1),4);


		//imwrite("04.bmp",Band_img);

		//AfxMessageBox("1");


		findContours( Band_img.clone(), contours, hierarchy, CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE );
		m_max_object_num = -1;m_max_object_value = 0;
		for( int k = 0; k < contours.size(); k++ )
		{  
			boundRect = boundingRect( Mat(contours[k]) );
		...
		if(m_max_object_num == -1)
		{
			Result_Info[Cam_num].Format("C1:00_-1_C1:01_-1_C2:01_-1");
			rectangle(Dst_Img[Cam_num],Rect(100,100,Out_binary.cols-200,Out_binary.rows-200),Scalar(0,0,255),2);
			return true;
		}

		Band_img = Mat::zeros(Band_img.size(), CV_8UC1);
		drawContours(Band_img, contours, m_max_object_num, CV_RGB(255,255,255), CV_FILLED, 8, hierarchy);
		//imwrite("05.bmp",Band_img);
		Band_img.copyTo(Filled_Outter_Circle); 
		int num2_in = countNonZero(Band_img);

		Mat Dist_Img, label, Tmp_Img;
		distanceTransform(Band_img, Dist_Img,label, CV_DIST_L2,CV_DIST_MASK_3);
		double m_max_distance;double m_min_distance;Point m_max_distance_Center;Point m_min_distance_Center;
		minMaxLoc(Dist_Img,&m_min_distance,&m_max_distance,&m_min_distance_Center,&m_max_distance_Center,Band_img);
		m_min_distance = 0;


		// Outter_Circle 안에 거리로 체크

		Mat Check_Inner_Ring = Outter_Circle.clone();
		J_Fill_Hole(Check_Inner_Ring);
		subtract(Check_Inner_Ring,Outter_Circle,Check_Inner_Ring);

		double circle_info1[5];
		J_Fitting_Ellipse(Band_img,circle_info1);

		//distanceTransform(Check_Inner_Ring, Dist_Img,label, CV_DIST_L2,CV_DIST_MASK_3);
		//double m_max_radius;Point m_max_radius_Center;
		//minMaxLoc(Dist_Img,NULL,&m_max_radius,NULL,&m_max_radius_Center,Check_Inner_Ring);

		Check_Inner_Ring = Mat::zeros(Check_Inner_Ring.size(), CV_8UC1);
		ellipse(Check_Inner_Ring,Point((int)circle_info1[0],(int)circle_info1[1]),Size(circle_info1[2]/2,circle_info1[3]/2),circle_info1[4],0,360,CV_RGB(255,255,255),1,1,0);
		//imwrite("05.bmp",Check_Inner_Ring);
		
		//ellipse(Dst_Img[Cam_num],Point((int)circle_info1[0],(int)circle_info1[1]),Size(circle_info1[2]/2,circle_info1[3]/2),circle_info1[4],0,360,CV_RGB(255,0,0),1,1,0);

		double num_in_total = countNonZero(Check_Inner_Ring);

		//imwrite("06.bmp",Band_img);

		dilate(Band_img,Band_img,element,Point(-1,-1),2);
		subtract(Check_Inner_Ring,Band_img,Check_Inner_Ring);
		//bitwise_and(Check_Inner_Ring,Band_img,Check_Inner_Ring);
		double num_in_connected = countNonZero(Check_Inner_Ring);
		double num_in = num_in_connected;
		
		//imwrite("07.bmp",Check_Inner_Ring);

		vector<vector<Point> > contours2;
		vector<Vec4i> hierarchy2;
		findContours( Check_Inner_Ring.clone(), contours2, hierarchy2, CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE );
		double m_max_object_num2 = -1;m_max_object_value = 0;
		for( int k = 0; k < contours2.size(); k++ )
		{  
			boundRect = boundingRect( Mat(contours2[k]) );
			// MBR 이 영상의 하단과 상단에 붙을 경우 에러 처리함.
			if(boundRect.x > 1 || boundRect.x + boundRect.width < Gray_Img[Cam_num].cols-1 || 
				boundRect.y > 1 || boundRect.y + boundRect.height < Gray_Img[Cam_num].rows-1 )
			{
		...
		//J_Fill_Hole(Filled_Outter_Circle);
		//dilate(Filled_Outter_Circle,Filled_Outter_Circle,element,Point(-1,-1),2);
		//erode(Filled_Outter_Circle,Filled_Outter_Circle,element,Point(-1,-1),2);
		//subtract(Filled_Outter_Circle,Band_img,Filled_Outter_Circle);



		//int num_in = countNonZero(Filled_Outter_Circle);
		...
		if (Result_Text_View)
		{
			drawContours(Dst_Img[Cam_num], contours, m_max_object_num, CV_RGB(0,100,255), CV_FILLED, 8, hierarchy);
			if (m_max_distance > 0)
			{
				circle(Dst_Img[Cam_num],m_max_distance_Center,m_max_distance,CV_RGB(255,100,0),2);
				msg.Format("Max(%1.3f)",2*m_max_distance*ALG_C1_Param.Res_X + ALG_C1_Param.Offset[1]);
				putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(m_max_distance_Center.x,m_max_distance_Center.y-20), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(255,100,0), 2, 8);
			}
			if (m_max_object_num2 != -1)
			{
				drawContours(Dst_Img[Cam_num], contours2, m_max_object_num2, CV_RGB(255,100,0),2, 8, hierarchy2);
				boundRect = boundingRect( Mat(contours2[m_max_object_num2]) );
				rectangle(Dst_Img[Cam_num],boundRect,CV_RGB(255,0,0),1,1);
				if (num_in >= 0)
				{
					//	//AfxMessageBox("1");
					//	//circle(Dst_Img[Cam_num],m_min_distance_Center,m_min_distance,CV_RGB(255,100,0),2);
					msg.Format("Cut(%1.3f)",num_in*ALG_C1_Param.Res_X + ALG_C1_Param.Offset[2]);
					putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(boundRect.x,boundRect.y-15), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(255,100,0), 2, 8);
				}
			} else
			{
				num_in = 0;
				msg.Format("Cut(0.000)");
				putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(out_circle_x-54,out_circle_y-100), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(0,150,255), 2, 8);
			}
			//if (m_min_distance == -1)
			//{
			...
			//	//AfxMessageBox("1");
			//	//circle(Dst_Img[Cam_num],m_min_distance_Center,m_min_distance,CV_RGB(255,100,0),2);
			//	msg.Format("Machining : NG");
			//	putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(out_circle_x-90,out_circle_y-100), FONT_HERSHEY_SIMPLEX, 0.8, CV_RGB(255,150,0), 2, 8);
			//	m_min_distance = 9999;
			//} else
			//{
			//	msg.Format("Machining : OK");
			//	putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(out_circle_x-90,out_circle_y-100), FONT_HERSHEY_SIMPLEX, 0.8, CV_RGB(0,150,255), 2, 8);
			//}
		}

		Result_Info[Cam_num].Format("C1:00_%1.4f_C1:01_%1.4f_C1:02_%1.4f",2*Gagong_Diameter*ALG_C1_Param.Res_X + ALG_C1_Param.Offset[0],2*m_max_distance*ALG_C1_Param.Res_X + ALG_C1_Param.Offset[1], num_in*ALG_C1_Param.Res_X + ALG_C1_Param.Offset[2]);

		return true;


		Mat Gol_Inspection_Circle = Mat::zeros(Outter_Circle.size(), CV_8UC1);
		circle(Gol_Inspection_Circle,Point((int)ALG_C1_Param.nBasicCenter.x,(int)ALG_C1_Param.nBasicCenter.y),ALG_C1_Param.nDistCentertoGol,CV_RGB(255,255,255),CV_FILLED,8);
		circle(Gol_Inspection_Circle,Point((int)ALG_C1_Param.nBasicCenter.x,(int)ALG_C1_Param.nBasicCenter.y),ALG_C1_Param.nDistCentertoDolgi-50,CV_RGB(0,0,0),CV_FILLED,8);
		dilate(Thres_Out_binary,Thres_Out_binary,element,Point(-1,-1),5);
		erode(Thres_Out_binary,Thres_Out_binary,element,Point(-1,-1),4);
		subtract(Gol_Inspection_Circle,Thres_Out_binary,Gol_Inspection_Circle);
		//imwrite("02.bmp",Gol_Inspection_Circle);

		Mat GolHull_Inspection_Circle = Mat::zeros(Outter_Circle.size(), CV_8UC1);
		findContours( Gol_Inspection_Circle.clone(), contours, hierarchy, CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE );
		m_max_object_num = -1;m_max_object_value = 0;
		...
		for( int k = 0; k < contours.size(); k++ )
		{  
			if (k != m_max_object_num)
			{
				m_area = contourArea(hull[k],false);
				if (m_area > 500)
				{
					drawContours( GolHull_Inspection_Circle, hull, k, Scalar(255), CV_FILLED, 8, vector<Vec4i>(), 0, Point() );
				}
			}
		}
		circle(GolHull_Inspection_Circle,Point((int)ALG_C1_Param.nBasicCenter.x,(int)ALG_C1_Param.nBasicCenter.y),ALG_C1_Param.nDistCentertoDolgi-20,CV_RGB(0,0,0),CV_FILLED,8);
		//imwrite("01.bmp",GolHull_Inspection_Circle);
		//imwrite("00.bmp",Gol_Inspection_Circle);
		subtract(GolHull_Inspection_Circle,Gol_Inspection_Circle,Gol_Inspection_Circle);
		//imwrite("02.bmp",Gol_Inspection_Circle);
		erode(Gol_Inspection_Circle,Gol_Inspection_Circle,element,Point(-1,-1),1);
		dilate(Gol_Inspection_Circle,Gol_Inspection_Circle,element,Point(-1,-1),1);
		//imwrite("02.bmp",Gol_Inspection_Circle);

		findContours( Gol_Inspection_Circle.clone(), contours, hierarchy, CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE );
		int defect_size = 0;double max_defect_size = 0;
		if (contours.size() > 0)
		{
			if (Result_Text_View)
			{
				...
				{  
					m_area = contourArea(contours[k],false);
					if (max_defect_size <= m_area)
					{
						max_defect_size = m_area;
					}
					boundRect = boundingRect( Mat(contours[k]) );
					drawContours( Dst_Img[Cam_num], contours, k, CV_RGB(255,0,255), CV_FILLED, 8, vector<Vec4i>(), 0, Point() );
					msg.Format("%1.0f",m_area + ALG_C1_Param.Offset[0]);
					putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(boundRect.x,boundRect.y-5), FONT_HERSHEY_SIMPLEX, 0.4, CV_RGB(255,0,255), 1, 8);
				}
			} else
			{
				for( int k = 0; k < contours.size(); k++ )
			...
			defect_size = (int)(max_defect_size + ALG_C1_Param.Offset[2]);//countNonZero(Gol_Inspection_Circle);
		} else
		{
			defect_size = (int)(max_defect_size + ALG_C1_Param.Offset[2]);
		}
		...
		Point2f m_distance_Center(0,0);
		double m_distance = 0;
		for (int i=(int)circle_info[0] - ALG_C1_Param.nDistCentertoDolgi - 10;i<(int)circle_info[0] + ALG_C1_Param.nDistCentertoDolgi + 10;i++)
		{
			for (int j=(int)circle_info[1] - ALG_C1_Param.nDistCentertoDolgi - 10;j<(int)circle_info[1] + ALG_C1_Param.nDistCentertoDolgi + 10;j++)
			{
				if (i<=0 || i>=Outter_Circle.cols || j<=0 || j>=Outter_Circle.rows)
				{
					continue;
				}
				if (Outter_Circle.at<uchar>(j,i) > 1)
				{
					px.push_back(i);
					py.push_back(j);
					if (Result_Text_View)
		...
		if (px.size() >= 10)
		{
			...
			if (ALG_C1_Param.nDistCentertoDolgi*0.7 <= (int)(c_ellipse.w + c_ellipse.h)/2)
			{
			...
			{
				m_distance_Center.x = ALG_C1_Param.nBasicCenter.x;
				m_distance_Center.y = ALG_C1_Param.nBasicCenter.y;
				m_distance = ALG_C1_Param.nDistCentertoDolgi -20;
			}
		...
		if (Result_Text_View)
		{
			...
			circle(Dst_Img[Cam_num],m_distance_Center,2,CV_RGB(200,100,0),3);
		}
		...
		if (Result_Text_View)
		{
			msg.Format("D(%1.3f)",Gagong_Diameter*ALG_C1_Param.Res_X + ALG_C1_Param.Offset[0]);
			putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(out_circle_x-40,out_circle_y - 130), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(255,100,0), 2, 8);
			msg.Format("dC(%1.3f)",t_d_center + ALG_C1_Param.Offset[1]);
			putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(out_circle_x-40,out_circle_y + 130), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(255,100,0), 2, 8);
		}
		//Rect Rect_Contour = boundingRect(contours[m_max_object_num]);
		if (Gagong_Diameter != 0)
		{
		...
		Result_Info[Cam_num].Format("C1:00_%1.4f_C1:01_%1.4f_C1:02_%d",Gagong_Diameter*ALG_C1_Param.Res_X + ALG_C1_Param.Offset[0],t_d_center + ALG_C1_Param.Offset[1], defect_size);
	} else
	{
		Result_Info[Cam_num].Format("C1:00_-1_C1:01_-1_C1:02_-1");
	}
	return true;
}
...
bool CImPro_Library::RUN_Algorithm_CAM2()
{
	...
	if (!Gray_Img[Cam_num].empty())
	{
		...
		if (t_thres > 255)
		{
			Result_Info[Cam_num].Format("C2:00_-1_C2:01_-1_C2:02_-1");
			rectangle(Dst_Img[Cam_num],Rect(100,100,Out_binary.cols-200,Out_binary.rows-200),Scalar(0,0,255),2);
			return true;
		} else
		{
		...
		J_Delete_Boundary(Inner_Circle,200);
		...
		{  
			...
			{
				if (m_max_object_value<= boundRect.width*boundRect.height 
					&& boundRect.width < 500 && boundRect.height < 500
					&& boundRect.width > 200 && boundRect.height > 200
					&& (double)boundRect.height/(double)boundRect.width < 1.2 
					&& (double)boundRect.height/(double)boundRect.width > 0.8)
				{
					m_max_object_value = boundRect.width*boundRect.height;
					m_max_object_num = k;
				}
		...
		J_Fill_Hole(Inner_Circle);
		Mat Inner_Circle_measure;
		subtract(Inner_Circle,Outter_Circle,Inner_Circle_measure);
		double Inner_Circle_Info[5] = {0,}; // 내경 : 중심X,Y,가로,세로,각도

		erode(Inner_Circle_measure,Inner_Circle_measure,element,Point(-1,-1),10);
		dilate(Inner_Circle_measure,Inner_Circle_measure,element,Point(-1,-1),10);

		//imwrite("01.bmp",Inner_Circle_measure);

		J_Fitting_Ellipse(Inner_Circle_measure, Inner_Circle_Info);
		double m_InnerCircle = (Inner_Circle_Info[2]+Inner_Circle_Info[3])/2;
		if (Result_Text_View)
		{
			circle(Dst_Img[Cam_num],Point(Inner_Circle_Info[0],Inner_Circle_Info[1]),m_InnerCircle/2,CV_RGB(100,100,255),2);
		}
		...
		if (Result_Text_View)
		{
			...
			msg.Format("D0(%1.3f)",0.5*(ALG_C2_Param.Res_X+ALG_C2_Param.Res_Y)*m_distance*2 + ALG_C2_Param.Offset[0]);
			...
			msg.Format("D1(%1.3f)",0.5*(ALG_C2_Param.Res_X+ALG_C2_Param.Res_Y)*m_OutCircle + ALG_C2_Param.Offset[1]);
			putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(Circle_Info[0]-60,Circle_Info[1] - (int)m_OutCircle/2 + 60), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(205,50,205), 2, 8);
			msg.Format("D2(%1.3f)",0.5*(ALG_C2_Param.Res_X+ALG_C2_Param.Res_Y)*m_InnerCircle + ALG_C2_Param.Offset[2]);
			putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(Circle_Info[0]-60,Circle_Info[1] - (int)m_InnerCircle/2 + 120), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(100,100,255), 2, 8);
		...
		Result_Info[Cam_num].Format("C2:00_%1.4f_C2:01_%1.4f_C2:02_%1.4f",0.5*(ALG_C2_Param.Res_X+ALG_C2_Param.Res_Y)*m_distance*2 + ALG_C2_Param.Offset[0],0.5*(ALG_C2_Param.Res_X+ALG_C2_Param.Res_Y)*m_OutCircle + ALG_C2_Param.Offset[1],0.5*(ALG_C2_Param.Res_X+ALG_C2_Param.Res_Y)*m_InnerCircle + ALG_C2_Param.Offset[2]);
	} else
	{
		Result_Info[Cam_num].Format("C2:00_-1_C2:01_-1_C2:02_-1");
	}
...
bool CImPro_Library::RUN_Algorithm_CAM3()
{
	...
	if (!Gray_Img[Cam_num].empty())
	{
		...
		Result_Info[Cam_num].Format("C3:00_%1.4f",(double)m_max_object_value+ALG_C3_Param.Offset[0]);
	} else
	{
		Result_Info[Cam_num].Format("C3:00_-1");
	}
...
bool CImPro_Library::RUN_Algorithm_CAM4()
{
	...
	if (!Gray_Img[Cam_num].empty())
	{
		...
		Mat Top_Defect = Mat::zeros(Size(Gray_Img[Cam_num].rows,Gray_Img[Cam_num].cols), CV_8U);
		Mat Top_Defect_Mor = Mat::zeros(Size(Gray_Img[Cam_num].rows,Gray_Img[Cam_num].cols), CV_8U);
		Mat Top_Defect_Mor2 = Mat::zeros(Size(Gray_Img[Cam_num].rows,Gray_Img[Cam_num].cols), CV_8U);
		boundRect.x = 0;boundRect.y = ALG_C4_Param.Top_y;boundRect.width = Gray_Img[Cam_num].cols-95;boundRect.height = ALG_C4_Param.Top_y2-ALG_C4_Param.Top_y+1;
		threshold(Gray_Img[Cam_num](boundRect),Top_Defect(boundRect),ALG_C4_Param.nMinMaxEdgeThreshold,255,CV_THRESH_BINARY_INV);
		boundRect.x = 0;boundRect.y = ALG_C4_Param.Top_y2;boundRect.width = Gray_Img[Cam_num].cols-95;boundRect.height = ALG_C4_Param.Bottom_y2-ALG_C4_Param.Top_y2+1;
		threshold(Gray_Img[Cam_num](boundRect),Top_Defect(boundRect),ALG_C4_Param.nMinMaxEdgeThreshold2,255,CV_THRESH_BINARY_INV);
		//imwrite("00.bmp",Top_Defect);

		//J_Delete_Boundary(Top_Defect,5);
		J_Fill_Hole(Top_Defect);
		//imwrite("01.bmp",Top_Defect);
		erode(Top_Defect,Top_Defect_Mor,element_v,Point(-1,-1),4);
		dilate(Top_Defect_Mor,Top_Defect_Mor,element_v,Point(-1,-1),4);
		subtract(Top_Defect,Top_Defect_Mor,Top_Defect_Mor);
		//imwrite("02.bmp",Top_Defect_Mor);

		//Mat element_hh = getStructuringElement(MORPH_RECT, Size(3, 1), Point(-1, -1) );

		dilate(Top_Defect,Top_Defect_Mor2,element_v,Point(-1,-1),4);
		erode(Top_Defect_Mor2,Top_Defect_Mor2,element_v,Point(-1,-1),4);
		subtract(Top_Defect_Mor2, Top_Defect,Top_Defect_Mor2);
		//imwrite("03.bmp",Top_Defect_Mor2);


		add(Top_Defect_Mor,Top_Defect_Mor2,Top_Defect_Mor);

		dilate(Top_Defect_Mor,Top_Defect_Mor,element,Point(-1,-1),3);
		erode(Top_Defect_Mor,Top_Defect_Mor,element,Point(-1,-1),3);

		//imwrite("04.bmp",Top_Defect_Mor);

		vector<vector<Point> > topcontours;
		vector<Vec4i> tophierarchy;

		findContours( Top_Defect_Mor.clone(), topcontours, tophierarchy, CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE );
		int m_max_object_value = 0;int m_max_object_num = -1;int t_area = 0;
		int t_min = 0;int t_min2 = 0;int t_max = 0;int t_max2 = 0;
		for( int k = 0; k < topcontours.size(); k++ )
		{  
			boundRect = boundingRect( Mat(topcontours[k]) );
			t_area = (int)(contourArea(topcontours[k],false)+0.5);
			if ((boundRect.y >= ALG_C4_Param.Top_y 
				&& boundRect.y <= ALG_C4_Param.Bottom_y) && t_area >= m_max_object_value)
			{
				if (boundRect.width > 1)
				{
					m_max_object_value = t_area;
					m_max_object_num = k;
					if (Result_Text_View)
					{
						boundRect.x -= 8;boundRect.y -= 8;
						boundRect.width += 16;boundRect.height += 16;
						rectangle(Dst_Img[Cam_num],boundRect, CV_RGB(255,100,0), 1, CV_AA);
						drawContours( Dst_Img[Cam_num], topcontours, k, CV_RGB(255,0,0), CV_FILLED, 8, vector<Vec4i>(), 0, Point() );
					}
				}
			}
		}
		...
		{
			boundRect = boundingRect( Mat(topcontours[m_max_object_num]) );
			t_max = (int)(contourArea(topcontours[m_max_object_num],false)+0.5);
			//t_max = boundRect.width*boundRect.height;
			if (Result_Text_View)
			{
				msg.Format("max(%1.3f)",ALG_C4_Param.Res_X*ALG_C4_Param.Res_X*(double)t_max + ALG_C4_Param.Offset[1]);
				putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(boundRect.x+boundRect.width + 15,boundRect.y), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(255,100,0), 2, 8);
			}
		}
		...
		{  
			boundRect = boundingRect( Mat(topcontours[k]) );
			t_area = (int)(contourArea(topcontours[k],false)+0.5);
			if ((boundRect.y >= ALG_C4_Param.Top_y2 
				&& boundRect.y <= ALG_C4_Param.Bottom_y2) && t_area >= m_max_object_value)
			{
				if (boundRect.width > 1)
				{
					m_max_object_value = t_area;
		...
		if (Result_Text_View)
		{
			line( Dst_Img[Cam_num], Point(0,ALG_C4_Param.Top_y2), Point(Dst_Img[Cam_num].cols,ALG_C4_Param.Top_y2), CV_RGB(255,0,100), 1, CV_AA);
			line( Dst_Img[Cam_num], Point(0,ALG_C4_Param.Bottom_y2), Point(Dst_Img[Cam_num].cols,ALG_C4_Param.Bottom_y2), CV_RGB(255,0,100), 1, CV_AA);
		}
		if (m_max_object_num != -1)
		{
			boundRect = boundingRect( Mat(topcontours[m_max_object_num]) );
			t_max2 = (int)(contourArea(topcontours[m_max_object_num],false)+0.5);
			//t_max2 = boundRect.width*boundRect.height;
			if (Result_Text_View)
			{
				msg.Format("max(%1.3f)",ALG_C4_Param.Res_X*ALG_C4_Param.Res_X*(double)t_max2 + ALG_C4_Param.Offset[3]);
				putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(boundRect.x+boundRect.width + 15,boundRect.y), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(255,100,0), 2, 8);
			}
		}
		...
		//	nLeft = -1;
		//	for(int j=100; j<Gray_Img[Cam_num].cols/2 ; j++)
		//	{
		//		if (j<0 || j >= Gray_Img[Cam_num].cols)
		//		{
		//			continue;
		...
		//	}
		//	nRight = -1;
		//	for(int j=Gray_Img[Cam_num].cols - 100; j>Gray_Img[Cam_num].cols/2 ; j--)
		//	{
		//		if (j<0 || j >= Gray_Img[Cam_num].cols)
		//		{
		//			continue;
		...
		//	line( Dst_Img[Cam_num], Point(0,ALG_C4_Param.Bottom_y), Point(Dst_Img[Cam_num].cols,ALG_C4_Param.Bottom_y), CV_RGB(0,255,0), 1, CV_AA);
			...
			//	msg.Format("min(%1.3f)",ALG_C4_Param.Res_X*(double)t_min + ALG_C4_Param.Offset[0]);
			//	putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(Gray_Img[Cam_num].cols/2-47,-10+ALG_C4_Param.Top_y + t_min_idx), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(255,255,0), 2, 8);
			//}
			//if (t_max_idx != -1)
			//{
			...
			//	msg.Format("max(%1.3f)",ALG_C4_Param.Res_X*(double)t_max + ALG_C4_Param.Offset[1]);
			//	putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(Gray_Img[Cam_num].cols/2-47,-10+ALG_C4_Param.Top_y + t_max_idx), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(255,255,0), 2, 8);
			//}
		//}
		...
		//int nLeft2 = -1;
		//int nRight2 = -1;
		//vector<int> nVec_MinMax_Left2;
		//vector<int> nVec_MinMax_Right2;
		//vector<int> nVec_MinMaxDist_Right2;
		////하단 폭 측정 
		//for(int i=ALG_C4_Param.Top_y2; i< ALG_C4_Param.Bottom_y2;i++)
		//{
		//	if (i<0 || i >= Gray_Img[Cam_num].rows)
		//	{
		//		continue;
		...
		//			continue;
		//		}
		//		if(CP_Gray_Img.at<uchar>(i,j)<=ALG_C4_Param.nMinMaxEdgeThreshold2)
		//		{
		//			nLeft2 = j;
		//			break;
		//		}
		...
		//		}
		//	}
		//	nVec_MinMax_Left2.push_back(nLeft2);
		//	nVec_MinMax_Right2.push_back(nRight2);
		//	nVec_MinMaxDist_Right2.push_back(nRight2-nLeft2);
		//}

		//int t_min2 = 999999;int t_min_idx2 = -1;
		//int t_max2 = 0;int t_max_idx2 = -1;

		//for (int i=0;i<nVec_MinMaxDist_Right2.size();i++)
		//{
		//	if (nVec_MinMaxDist_Right2[i] < t_min2)
		//	{
		//		t_min2 = nVec_MinMaxDist_Right2[i];
		//		t_min_idx2 = i;
		//	}
		//	if (nVec_MinMaxDist_Right2[i] > t_max2)
		//	{
		//		t_max2 = nVec_MinMaxDist_Right2[i];
		//		t_max_idx2 = i;
		//	}
		//}
		...
		//	line( Dst_Img[Cam_num], Point(0,ALG_C4_Param.Bottom_y2), Point(Dst_Img[Cam_num].cols,ALG_C4_Param.Bottom_y2), CV_RGB(255,0,100), 1, CV_AA);
			...
			//if (t_min_idx2 != -1)
			//{
			//	J_DrawArrow(Dst_Img[Cam_num], Point(nVec_MinMax_Left2[t_min_idx2],ALG_C4_Param.Top_y2 + t_min_idx2), Point(nVec_MinMax_Right2[t_min_idx2],ALG_C4_Param.Top_y2 + t_min_idx2), CV_RGB(255,255,0), 10 , 1, CV_AA, 0);
			//	J_DrawArrow(Dst_Img[Cam_num], Point(nVec_MinMax_Right2[t_min_idx2],ALG_C4_Param.Top_y2 + t_min_idx2), Point(nVec_MinMax_Left2[t_min_idx2],ALG_C4_Param.Top_y2 + t_min_idx2), CV_RGB(255,255,0), 10 , 1, CV_AA, 0);
			//	msg.Format("min(%1.3f)",ALG_C4_Param.Res_X*(double)t_min2 + ALG_C4_Param.Offset[2]);
			//	putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(Gray_Img[Cam_num].cols/2-47,-10+ALG_C4_Param.Top_y2 + t_min_idx2), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(255,255,0), 2, 8);
			//}
			//if (t_max_idx2 != -1)
			//{
			//	J_DrawArrow(Dst_Img[Cam_num], Point(nVec_MinMax_Left2[t_max_idx2],ALG_C4_Param.Top_y2 + t_max_idx2), Point(nVec_MinMax_Right2[t_max_idx2],ALG_C4_Param.Top_y2 + t_max_idx2), CV_RGB(255,255,0), 10 , 1, CV_AA, 0);
			//	J_DrawArrow(Dst_Img[Cam_num], Point(nVec_MinMax_Right2[t_max_idx2],ALG_C4_Param.Top_y2 + t_max_idx2), Point(nVec_MinMax_Left2[t_max_idx2],ALG_C4_Param.Top_y2 + t_max_idx2), CV_RGB(255,255,0), 10 , 1, CV_AA, 0);
			//	msg.Format("max(%1.3f)",ALG_C4_Param.Res_X*(double)t_max2 + ALG_C4_Param.Offset[3]);
			//	putText(Dst_Img[Cam_num], msg.GetBuffer(), Point(Gray_Img[Cam_num].cols/2-47,-10+ALG_C4_Param.Top_y2 + t_max_idx2), FONT_HERSHEY_SIMPLEX, 0.6, CV_RGB(255,255,0), 2, 8);
			//}
		//}
		//Result_Info[Cam_num].Format("C4:00_%1.3f_C4:01_%d_C4:02_%d", t_center,t_min,t_max);
		Result_Info[Cam_num].Format("C4:00_%1.4f_C4:01_%1.4f_C4:02_%1.4f_C4:03_%1.4f", ALG_C4_Param.Res_X*(double)t_min + ALG_C4_Param.Offset[0],ALG_C4_Param.Res_X*(double)t_max + ALG_C4_Param.Offset[1]
		,ALG_C4_Param.Res_X*(double)t_min2 + ALG_C4_Param.Offset[2],ALG_C4_Param.Res_X*(double)t_max2 + ALG_C4_Param.Offset[3]);
	} else
	{
		Result_Info[Cam_num].Format("C4:00_-1_C4:01_-1_C4:02_-1_C4:03_-1");
	}
	return true;}
