...
float logpolar(Mat& src, Mat& dst)
{
    ...
    float log_base = pow(10.0f, log10(d) / radii);
...
RotatedRect LogPolarFFTTemplateMatch(Mat& im0, Mat& im1, double canny_threshold1, double canny_threshold2)
{
    ...
    Mat F0[2], F1[2];
    Mat f0, f1, h0, h1, f0lp, f1lp;;
    float log_base0, log_base1;
#pragma omp parallel sections num_threads(2)  
    {  
        #pragma omp section  
		{
            ...
            highpass(f0.size(), h0);
            f0 = f0.mul(h0);
            log_base0 = logpolar(f0, f0lp);
        }
        #pragma omp section  
		{
		    Canny(im1, im1, canny_threshold1, canny_threshold2);
			//Mat t_im1 = Mat::zeros(im1.size(), CV_8UC1);
			//im1(ROI).copyTo(t_im1(ROI));
			//im1 = t_im1.clone();
			//imwrite("01.bmp",im1);
            ...
            highpass(f1.size(), h1);
            f1 = f1.mul(h1);
            log_base1 = logpolar(f1, f1lp);
        }
    }  

    //double t_score = compareHist(f0, f1, HISTCMP_BHATTACHARYYA);
    //CString msg;
    //msg.Format(L"%1.2f", t_score);
    //AfxMessageBox(msg);
    ...
    float scale = pow(log_base0, (float)rotation_and_scale.x);
    // --------------
    if (scale > 1.8)
    {
    ...
    //imwrite("00.bmp", im1 * 255);
    //imwrite("01.bmp", im1_rs * 255);
    // find translation
